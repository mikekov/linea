...
using Linea::set_scaled_dash;
using Linea::calc_scale_line_width;
using Inkscape::UI::EditOperation;
...
void set_item_style(SPItem* item, SPCSSAttr* css) {
    // Text objects store style on their child tspans, not just the root.
    // Applying CSS only to the root <text> element is a no-op when tspans
    // override the property. Use sp_desktop_apply_css_recursive to propagate
    // the style down the tree, matching the classic stroke-style.cpp path.
...
void set_stroke_width(SPItem* item, double width_typed, bool hairline, const ::Inkscape::Util::Unit* unit) {
