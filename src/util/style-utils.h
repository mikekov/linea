...
#ifndef LINEA_STYLE_UTILS_H
#define LINEA_STYLE_UTILS_H
...
// Only a subset of styles is queried and returned in the PresentationState struct.
...
struct PaintProp {
    Inkscape::UI::Widget::PaintMode mode = Inkscape::UI::Widget::PaintMode::None;
    ...
    std::optional<Inkscape::UI::Widget::PaintDerivedMode> derived_mode;
...
struct PresentationState {
    ...
    // --- paint order ---
    mixed_property<SPIPaintOrder> paint_order;
    ...
    // --- render ---
    mixed_property<SPImageRendering> image_rendering{SP_CSS_IMAGE_RENDERING_AUTO};
};

// TODO: difference between two PresentationState objects
struct PresentationStateDelta {
    // Add fields as needed for delta tracking
};
...
void query_style_impl(PresentationState& props, SPItem* item, StyleQueryFlags flags);
} // namespace detail

// Query style properties from a single item.
inline PresentationState query_style_properties(SPItem* item,
                                              StyleQueryFlags flags = StyleQueryFlags::None) {
    PresentationState props;
    if (item) {
...
PresentationState query_style_properties(Range&& items,
                                       StyleQueryFlags flags = StyleQueryFlags::None) {
    PresentationState props;
...
using CssPtr = std::unique_ptr<SPCSSAttr, ::Inkscape::Util::Deleter<sp_repr_css_attr_unref>>;

// Create an empty SPCSSAttr wrapped in a CssPtr.
inline CssPtr make_css() { return CssPtr(sp_repr_css_attr_new()); }

SPObject* getMarkerObj(const char* n, SPDocument* doc);
...
double calc_scale_line_width(double width_typed, const SPItem* item, const ::Inkscape::Util::Unit* unit);

} // namespace Linea

#endif // LINEA_STYLE_UTILS_H
