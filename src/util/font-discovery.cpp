...
int get_family_font_index(const std::vector<FontInfo>& family) {
    assert(!family.empty());
    ...
    if (it != end(family)) {
        return std::distance(begin(family), it);
    }
    return family.empty() ? -1 : 0;
}

const FontInfo& get_family_font(const std::vector<FontInfo>& family) {
    assert(!family.empty());
    auto idx = get_family_font_index(family);
    return family.at(idx);
    // auto it = std::ranges::find_if(family, [](auto& fam) {
    //     return fam.face &&
    //         (fam.face->get_name().raw().find("Regular") != std::string::npos ||
    //          fam.face->get_name().raw().find("Normal")  != std::string::npos);
...
const std::vector<FontInfo>* find_font_family(const std::vector<std::vector<FontInfo>>& fonts, const Glib::ustring& family_name) {
    auto lower = family_name.lowercase();
    for (auto& family : fonts) {
        if (!family.empty() && family.front().ff && family.front().ff->get_name().lowercase() == lower) {
            return &family;
        }
    }
    return nullptr;
...
const FontInfo* find_font_face(const std::vector<FontInfo>& family, const Glib::ustring& style_name) {
...
const FontInfo* find_closest_face(const std::vector<FontInfo>& family, const Pango::FontDescription& target) {
    const FontInfo* best = nullptr;
    int best_score = std::numeric_limits<int>::max();
    for (auto& font : family) {
        if (!font.face) continue;
        auto desc = font.face->describe();
        // weight dominates (100..1000 scale); style and stretch mismatches
        // are penalized so exact-trait faces win over near-weight ones
        int score = std::abs(static_cast<int>(desc.get_weight()) - static_cast<int>(target.get_weight()))
                  + (desc.get_style() == target.get_style() ? 0 : 2000)
                  + std::abs(static_cast<int>(desc.get_stretch()) - static_cast<int>(target.get_stretch())) * 200;
        if (score < best_score) {
            best_score = score;
            best = &font;
        }
    }
    if (!best && !family.empty()) {
        best = &get_family_font(family);
    }
    return best;
}
