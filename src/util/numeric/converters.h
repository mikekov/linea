 ...
 */
inline double read_number(const char* value, bool warning = true)
{
    if (!value) {
...
inline bool read_bool(const char* value, bool default_value)
{
    if (!value)
 ...
 */
inline std::vector<double> read_vector(const char *value)
{
    std::vector<double> v;

    const char *beg = value;
...
inline QString formatTrimmed(double value, int prec) {
