...
Cairo::RefPtr<Cairo::Surface> draw_symbol(SPObject& symbol, double box_w, double box_h, double device_scale, SPDocument* preview_document, bool style_from_use, bool allow_upscale) {
    ...
    bool add_cross)
{
    ...
    if (!strncmp (mstroke, "url(", 4)) {
        // QT TODO: getMarkerObj from stroke-style.cpp not compiled in Qt port
        SPObject *linkObj = nullptr; // getMarkerObj(mstroke, source);
        if (linkObj) {
...
// Uncomment this to get the sandbox documents saved (useful for debugging)
    ...
    QColor fg = marker_color;
    auto const qcolor_to_css = [](QColor const &c) {
        auto const u32 = static_cast<uint32_t>(c.red()) << 24
                       | static_cast<uint32_t>(c.green()) << 16
                       | static_cast<uint32_t>(c.blue()) << 8
                       | static_cast<uint32_t>(c.alpha());
        return Glib::ustring(Inkscape::Colors::rgba_to_hex(u32, true));
    };
    auto fgcolor = qcolor_to_css(fg);
    fg.setRedF(1 - fg.redF());
    fg.setGreenF(1 - fg.greenF());
    fg.setBlueF(1 - fg.blueF());
    auto bgcolor = qcolor_to_css(fg);
...
Cairo::RefPtr<Cairo::Surface> object_renderer::render_object(SPObject& object, double width, double height, double device_scale, object_renderer::options opt) {
    ...
    if (is<SPSymbol>(&object)) {
        ...
        surface = draw_symbol(object, width, height, device_scale, _symbol_document.get(), opt._symbol_style_from_use, opt._symbol_allow_upscale);
    }
        ...
        surface = PatternManager::get().get_image_surface(pattern, width, height, device_scale);
    }
...
QImage object_renderer::render(SPObject& object, double width, double height, double device_scale, options options) {
    auto surface = render_object(object, width, height, device_scale, options);
