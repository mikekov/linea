...
TypographyState query_text_properties(const std::vector<SPItem*>& items) {
    ...
    for (auto item : items) {
        ...
        // --- decoration thickness (not inherited — check parent if unset) ---
        {
            ...
            auto& tdt = *tdt_ptr;
            if (tdt.set) {
                props.decoration_thickness.merge(DecorationThicknessState{
...
void fill_css_from_fontspec(SPCSSAttr* css, const Glib::ustring& fontspec) {
    ...
    // for trailing weight numbers and re-adding spaces after commas).
    PangoFontDescription *descr = pango_font_description_from_string(spec.c_str());
    ...
    // PANGO BUG: a spec like "Delicious, 500 Italic" parses family as
    // "Delicious, 500". Chop trailing ",[1-9]00" off the family.
    Glib::RefPtr<Glib::Regex> weight = Glib::Regex::create(",[1-9]00$");
    Family = weight->replace(Family, 0, "", Glib::Regex::MatchFlags::PARTIAL);

    // Pango removes the space after commas in family lists; put it back.
    size_t i = 0;
    ...
    fill_css_from_font_description(css, Family, desc, spec);
}
...
std::vector<SPItem*> get_all_text_spans(Inkscape::Selection* selection) {
    ...
    for (auto& object : selection->objects()) {
        if (is_textual_item(object)) {
            auto vec = get_text_spans(cast<SPItem>(object));
            std::copy(begin(vec), end(vec), std::back_inserter(items));
        }
    }
