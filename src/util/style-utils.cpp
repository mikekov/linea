...
#include "preferences.h" //TODO - remove
...
void merge_item_style(PresentationState& props, SPItem* item) {
    auto style = item->style;
    if (!style) return;

    // --- fill ---
    if (!props.fill.is_mixed()) {
        props.fill.merge(classify_paint(style->fill, style->fill_opacity.as_double()));
    }
    ...
    if (!props.stroke.is_mixed()) {
        props.stroke.merge(classify_paint(style->stroke, style->stroke_opacity.as_double()));
    }
    ...
    // --- render ---
    props.image_rendering.merge(style->image_rendering.value);
}
