...
void FontCollections::init()
{
    ...
    // g_mkdir_with_parents is portable, idempotent, and creates parent dirs.
    if (g_mkdir_with_parents(directory.c_str(), 0755) == -1) {
        int problem = errno;
        g_warning("Unable to create font collections directory (%s) (%d)",
                  g_strerror(problem), problem);
    }
