...
#ifndef LINEA_TEXT_UTILS_H
#define LINEA_TEXT_UTILS_H
...
struct DecorationThicknessState {
    double value = 0;
    bool auto_val = true;
    bool from_font = false;
    bool operator == (const DecorationThicknessState&) const = default;
};
...
struct TypographyState {
    ...
    mixed_property<DecorationThicknessState> decoration_thickness;
};
...
TypographyState query_text_properties(const std::vector<SPItem*>& items);
...
// Fill CSS attributes from a fontspec string (e.g. "Family, Style"). Extracts the
// family via Pango (matching FontLister::ui_from_fontspec) and delegates to
// fill_css_from_font_description. If fontspec is empty, "sans-serif" is used.
// Replaces FontLister::fill_css without requiring the FontLister singleton.
void fill_css_from_fontspec(SPCSSAttr* css, const Glib::ustring& fontspec);
...
bool apply_text_dx(Inkscape::UI::Tools::TextTool* tool, SPDesktop* desktop, double new_dx);
...
bool apply_text_char_rotation(Inkscape::UI::Tools::TextTool* tool, SPDesktop* desktop, double new_degrees);
...
std::optional<double> query_text_char_rotation(Inkscape::UI::Tools::TextTool* tool);
...
void apply_text_css(SPItem* text_item, Inkscape::UI::Tools::TextTool* tool, SPCSSAttr* css);

// --- Unit helpers for font-size / line-height ---

// True if unit is relative (unitless/em/ex/%).
bool is_relative_unit(Inkscape::Util::Unit const *unit);
bool is_relative_unit(int css_unit);

// Convert a Unit abbreviation to SP_CSS_UNIT_xx.
int unit_to_css_unit(Inkscape::Util::Unit const *unit);
...
double convert_lineheight_between_units(double value, int old_css_unit,
                                        Inkscape::Util::Unit const *new_unit, double avg_font_size);
...
std::string format_line_height_css(double value, Inkscape::Util::Unit const *unit);


// Return all text spans in a text object
std::vector<SPItem*> get_all_text_spans(SPItem* text);

// ditto, but in a selection
std::vector<SPItem*> get_all_text_spans(Inkscape::Selection* selection);

// true if obj represents a textual element
bool is_textual_item(const SPObject* obj);

} // namespace

#endif // LINEA_TEXT_UTILS_H
