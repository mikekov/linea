...
VectorParam::param_newWidget()
{
    ...
    auto x_spin = new Linea::UI::NumberEdit();
    x_spin->setValue(vector[Geom::X]);
    x_spin->setLabel(QObject::tr("X"));
    layout->addWidget(x_spin, 1);

    auto y_spin = new Linea::UI::NumberEdit();
    y_spin->setValue(vector[Geom::Y]);
    y_spin->setLabel(QObject::tr("Y"));
    layout->addWidget(y_spin, 1);

    QObject::connect(x_spin, &Linea::UI::NumberEdit::valueChanged, [this](double v) {
        vector[Geom::X] = v;
        set_and_write_new_values(origin, vector);
    });
    QObject::connect(y_spin, &Linea::UI::NumberEdit::valueChanged, [this](double v) {
        vector[Geom::Y] = v;
        set_and_write_new_values(origin, vector);
    });
