...
PointParam::param_newWidget()
{
    ...
    auto create = [this, layout]() {
        auto spin = create_param_spin();
    ...
    x_spin->setValue((*this)[Geom::X]);

    auto y_spin = create();
    y_spin->setLabel(QObject::tr("Y"));
    y_spin->setValue((*this)[Geom::Y]);

    QObject::connect(x_spin, &Linea::UI::NumberEdit::valueChanged, [this](double) { on_value_changed(); });
    QObject::connect(y_spin, &Linea::UI::NumberEdit::valueChanged, [this](double) { on_value_changed(); });
