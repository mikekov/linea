...
public:
    ...
    // This creates a new widget; caller takes ownership.
    virtual QWidget* param_newWidget() = 0;
    ...
    Glib::ustring param_prefix;  ///< Short prefix (e.g. "X", "Y") shown when paired in a group
    ...
    static QString normalize_label_text(const Glib::ustring& text);
    static QLabel* create_param_label(const Glib::ustring& text);
    Linea::UI::NumberEdit* create_param_spin() const;
protected:
    std::tuple<QWidget*, QVBoxLayout*, QHBoxLayout*> create_param_container(const Glib::ustring& label = {});

    int _default_width = 0;
...
class ScalarParam : public Parameter {
  public:
    ScalarParam(const Glib::ustring &label, const Glib::ustring &tip, const Glib::ustring &key,
                Inkscape::UI::Widget::Registry *wr, Effect *effect, double default_value = 1.0, const Glib::ustring& unit = {});
