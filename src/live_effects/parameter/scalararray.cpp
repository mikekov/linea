...
QWidget* ScalarArrayParam::param_newWidget()
{
    ...
    if (add_slider) {
        ...
        scale->setValue(_vector[_active_index]);

        QObject::connect(scale, &Linea::UI::SpinScale::valueChanged, [this](double v) {
            if (_active_index >= _vector.size()) return;
            double val = (v < 1e-6 && v > -1e-6) ? 0.0 : v;
        ...
        spin->setValue(_vector[_active_index]);
...
void
ScalarArrayParam::on_value_changed()
{
    // No longer needed — signal handlers are inline lambdas
}
