...
std::tuple<QWidget*, QVBoxLayout*, QHBoxLayout*> Parameter::create_param_container(const Glib::ustring& label) {
    auto widget = new QWidget();
    auto layout = new QVBoxLayout(widget);
    if (!label.empty()) {
        layout->addWidget(create_param_label(label));
    }
    ...
    layout->setSpacing(0);
    return std::make_tuple(widget, layout, hbox);
}

QString Parameter::normalize_label_text(const Glib::ustring& text) {
    // Convert the first '_' into a Qt keyboard accelerator ('&') and strip
    // a trailing ':' so labels read cleanly while still supporting alt-key
    // activation of buddy widgets.
    auto str = QString::fromStdString(text.raw());
...
QLabel* Parameter::create_param_label(const Glib::ustring& text) {
    auto label = new QLabel(normalize_label_text(text));
    label->setProperty("class", "panel-label");
    return label;
}

Linea::UI::NumberEdit* Parameter::create_param_spin() const {
    auto spin = new Linea::UI::NumberEdit();
    spin->setRange(1e-6, 1e6);
    spin->setDecimals(6);
    spin->setMinimumWidth(_default_width);
    return spin;
}
...
ScalarParam::ScalarParam(const Glib::ustring &label, const Glib::ustring &tip, const Glib::ustring &key,
                         ...
                         double const default_value, const Glib::ustring& unit)
...
QWidget* ScalarParam::param_newWidget() {
    if (!widget_is_visible) return nullptr;

    if (add_slider) {
        auto scale = new Linea::UI::SpinScale();
        scale->setRange(min, max);
        scale->setDecimals(digits);
        scale->numberEdit()->setSingleStep(inc_step);
        scale->numberEdit()->setMinimumWidth(70);
        scale->setValue(value);
        scale->setMinimumWidth(_default_width);

        QObject::connect(scale, &Linea::UI::SpinScale::valueChanged, [this](double v) {
            param_set_value(v);
            param_write_to_repr(param_getSVGValue().c_str());
        });
        return scale;
        ...
        spin->setSingleStep(inc_step);
        spin->setValue(value);
        spin->setSuffix(QString::fromStdString(_unit));
        if (_width_chars > 0) {
            spin->setMinimumWidth(_width_chars * 8);
        }
        if (_default_width > 0) {
            spin->setMinimumWidth(_default_width);
        }

        QObject::connect(spin, &Linea::UI::NumberEdit::valueChanged, [this](double v) {
            param_set_value(v);
            param_write_to_repr(param_getSVGValue().c_str());
        });
        return spin;
