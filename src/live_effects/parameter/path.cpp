...
PathParam::param_newWidget()
{
    ...
    auto add_button = [&](const char* icon_name, const char* tooltip, auto callback) {
        auto btn = new QPushButton();
        btn->setIcon(QIcon(QLatin1String(icon_name)));
        btn->setToolTip(QObject::tr(tooltip));
        QObject::connect(btn, &QPushButton::clicked, widget, callback);
        layout->addWidget(btn);
    };

    if (_edit_button) {
        add_button(":/icons/tool-node-editor", "Edit on-canvas", [this] { on_edit_button_click(); });
    }
    if (_copy_button) {
        add_button(":/icons/edit-copy", "Copy path", [this] { on_copy_button_click(); });
    }
    if (_paste_button) {
        add_button(":/icons/edit-paste", "Paste path", [this] { on_paste_button_click(); });
    }
    if (_link_button) {
        ...
        add_button(":/icons/edit-clone", "Link to path in clipboard", [this] { on_link_button_click(); });
    }
