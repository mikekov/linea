...
LPEMeasureSegments::LPEMeasureSegments(LivePathEffectObject *lpeobject) :
    ...
    // active for 1.1
    smallx100(_("Multiply values less than 1"), _("Multiply values smaller than 1 by 100 and leave out the unit"), "smallx100", &wr, this, false),
...
std::vector<PlanNode> LPEMeasureSegments::getPlan() {
    return {
        notebook({
            page(_("General"), {
                row({
                ...
                p(&hide_back, 2),
                p(&smallx100, 2),
                p(&avoid_overlapping, 2),
            }),
            page(_("Projection"), {
                p(&active_projection, 2),
                p(&linked_items, 2),
                row({
                    p(&distance_projection),
                    p(&angle_projection),
                }),
                p(&onbbox, 2),
                p(&bboxonly, 2),
            ...
            //     p(&helpdata),
            // }),
        }, pagenumber, [this](int idx) { pagenumber = idx; }),
    };
}
...
LPEMeasureSegments::createTextLabel(Geom::Point &pos, size_t counter, double length, Geom::Coord angle, bool remove, bool valid)
{
    ...
    Linea::fill_css_from_fontspec(css, fontbutton_str);
