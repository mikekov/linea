...
std::vector<PlanNode> LPEFilletChamfer::getPlan() {
    // Params in registration order, with radius and chamfer_steps getting
    // extra on_changed callbacks (matching the old GTK newWidget behavior).
    return {
        p(&nodesatellites_param),
        row({
            p(&radius, [this]() { updateAmount(); }),
            p(&unit),
        }),
        p(&method),
        p(&chamfer_steps, [this]() { updateChamferSteps(); }),
        p(&flexible, 2),
        p(&use_knot_distance, 2),
        p(&apply_no_radius, 2),
        p(&apply_with_radius, 2),
        p(&only_selected, 2),
        p(&hide_knots, 2),
        // Fillet / Inverse fillet buttons
        row({
            btn(_("Fillet"), [this]() { updateNodeSatelliteType(FILLET); }),
            btn(_("Inverse fillet"), [this]() { updateNodeSatelliteType(INVERSE_FILLET); }),
        }),
        // Chamfer / Inverse chamfer buttons
        row({
            btn(_("Chamfer"), [this]() { updateNodeSatelliteType(CHAMFER); }),
            btn(_("Inverse chamfer"), [this]() { updateNodeSatelliteType(INVERSE_CHAMFER); }),
        }),
    };
}
