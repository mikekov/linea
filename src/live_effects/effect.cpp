...
/**
 * This *creates* a widget. Deletion should be done by the eventual parent, or otherwise the caller.
 */
QWidget* Effect::newWidget() {
    auto vbox = new QWidget();
    auto layout = new QGridLayout(vbox);
    ...
    for (auto param : param_vector) {
        ...
        if (!param->param_label.empty() && !dynamic_cast<BoolParam*>(param)) {
            auto label = Parameter::create_param_label(param->param_label);
            label->setBuddy(widg);
            layout->addWidget(label, row++, 0);
        }
        widg->setEnabled(param->widget_is_enabled);
        layout->addWidget(widg, row++, 0);

        if (auto const tip = param->param_getTooltip()) {
            widg->setToolTip(QString::fromStdString(tip->raw()));
        }
    }
...
/// Recursively build widgets from a plan tree into a parent layout.
static std::pair<int, int> buildPlanNodes(const std::vector<PlanNode>& nodes, QGridLayout* parent, int row, int col, bool vertical);

static std::pair<int, int> buildPlanNode(const PlanNode& node, QGridLayout* parent, int row, int col) {
    std::visit([&](const auto& n) {
        using T = std::decay_t<decltype(n)>;
        if constexpr (std::is_same_v<T, PlanParam>) {
            if (!n.param || !n.param->widget_is_visible) return;
            auto widg = n.param->param_newWidget();
            if (!widg) return;
            widg->setEnabled(n.param->widget_is_enabled);

            int span = n.colspan;
            if (!n.param->param_label.empty() && !dynamic_cast<BoolParam*>(n.param)) {
                auto label = Parameter::create_param_label(n.param->param_label);
                label->setBuddy(widg);
                parent->addWidget(label, row++, col, 1, span);
            }
            parent->addWidget(widg, row++, col, 1, span);
            if (const auto tip = n.param->param_getTooltip()) {
                widg->setToolTip(QString::fromStdString(tip->raw()));
            }
            if (n.on_changed) {
                // Connect on_changed to valueChanged for scalar-like widgets.
                if (auto spin = dynamic_cast<Linea::UI::NumberEdit*>(widg)) {
                    QObject::connect(spin, &Linea::UI::NumberEdit::valueChanged, n.on_changed);
                } else if (auto scale = dynamic_cast<Linea::UI::SpinScale*>(widg)) {
                    QObject::connect(scale, &Linea::UI::SpinScale::valueChanged, n.on_changed);
                }
            }
            col += span;
        } else if constexpr (std::is_same_v<T, PlanButton>) {
            auto btn = new QPushButton();
            if (!n.label.empty()) {
                btn->setText(QString::fromStdString(n.label.raw()));
            }
            if (!n.icon.empty()) {
                btn->setIcon(QIcon(QString(":/icons/") + QString::fromStdString(n.icon)));
            }
            if (!n.tooltip.empty()) {
                btn->setToolTip(QString::fromStdString(n.tooltip.raw()));
            }
            if (n.checkable) {
            ...
            if (n.callback) {
                QObject::connect(btn, &QPushButton::clicked, n.callback);
            }
            parent->addWidget(btn, row++, col, 1, n.colspan);
            col += n.colspan;
        } else if constexpr (std::is_same_v<T, PlanRadioGroup>) {
            // Mutually-exclusive toggle buttons in a horizontal row.
            if (!n.label.empty()) {
                auto label = Parameter::create_param_label(n.label);
            ...
            buildPlanNodes(n.items, grid, 0, 0, n.vertical);
            for (int i = 0; i < grid->count(); ++i) {
                if (auto item = grid->itemAt(i)) {
                    if (auto button = dynamic_cast<QPushButton*>(item->widget())) {
                        group->addButton(button);
                    }
                }
            }
            hbox->addStretch();
            parent->addLayout(hbox, row++, col, 1, n.colspan);
            col += n.colspan;
        } else if constexpr (std::is_same_v<T, PlanGroup>) {
            if (!n.label.empty()) {
                auto label = Parameter::create_param_label(n.label);
                parent->addWidget(label, row++, col);
            }
            auto [newRow, newCol] = buildPlanNodes(n.children, parent, row, col, n.vertical);
            row = newRow;
            col = newCol;
        } else if constexpr (std::is_same_v<T, PlanGap>) {
            auto spacer = new QSpacerItem(n.size, n.size, QSizePolicy::Fixed, QSizePolicy::Fixed);
            parent->addItem(spacer, row++, col++);
        } else if constexpr (std::is_same_v<T, PlanNotebook>) {
            auto tabs = new QTabWidget();
            for (const auto& page : n.pages) {
                auto pageWidget = new QWidget();
                auto vbox = new QVBoxLayout(pageWidget);
                auto pageLayout = new QGridLayout();
                vbox->addLayout(pageLayout);
                pageLayout->setContentsMargins(0, 0, 0, 0);
                pageLayout->setHorizontalSpacing(4);
                pageLayout->setVerticalSpacing(4);
                auto [rows, cols] = buildPlanNodes(page.children, pageLayout, 0, 0, true);
                // set column stretch based on actual columns used...
                for (int c = 0; c < cols; ++c) {
                    pageLayout->setColumnStretch(c, 1);
                }
                vbox->addStretch();
                tabs->addTab(pageWidget, QString::fromStdString(page.label.raw()));
            }
            tabs->setCurrentIndex(n.current_page);
            if (n.on_page_changed) {
                QObject::connect(tabs, &QTabWidget::currentChanged, [n](int idx) { n.on_page_changed(idx); });
            }
            parent->addWidget(tabs, row++, col++);
        }
    }, node);
...
static std::pair<int, int> buildPlanNodes(const std::vector<PlanNode>& nodes, QGridLayout* parent, int row, int col, bool vertical) {
    ...
    for (auto const& node : nodes) {
        auto [newRow, newCol] = buildPlanNode(node, parent, row, col);
        if (vertical) { row = newRow; } else { col = newCol; }
        if (newRow > finalRow) finalRow = newRow;
        if (newCol > finalCol) finalCol = newCol;
    }
    return std::make_pair(finalRow, finalCol);
}

QWidget* Effect::buildUI() {
    auto plan = getPlan();
    if (plan.empty()) return newWidget();
    ...
    auto [rows, cols] = buildPlanNodes(plan, layout, 0, 0, true);
    for (int c = 0; c < cols; ++c) {
