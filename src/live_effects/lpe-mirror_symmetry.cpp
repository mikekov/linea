...
std::vector<PlanNode> LPEMirrorSymmetry::getPlan()
{
    // split_open is intentionally omitted — it was skipped in the old GTK UI too.
    return {
        p(&lpesatellites),
        p(&mode, 2),
        p(&discard_orig_path, 2),
        p(&fuse_paths, 2),
        p(&oposite_fuse, 2),
        p(&split_items, 2),
        p(&link_styles, 2),
        ...
        p(&center_point, 2),
        row({
            btn(_("Vertical center"), [this]() { centerVert(); }),
            btn(_("Horizontal center"), [this]() { centerHoriz(); }),
        }),
    };
}
