...
std::vector<PlanNode> LPESlice::getPlan() {
    return {
        row({
            btn(_("Reset styles"), [this]() { resetStyles(); }),
            btn(_("Vertical"), [this]() { centerVert(); }),
            btn(_("Horizontal"), [this]() { centerHoriz(); }),
        }),
        p(&lpesatellites, 3),
        p(&allow_transforms, 3),
        p(&start_point, 3),
        p(&end_point, 3),
        p(&center_point, 3),
    };
