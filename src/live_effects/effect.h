...
public:
    ...
    /// Build the UI from a declarative plan. Returns nullptr if no plan is provided.
    QWidget* buildUI();

    /// Override to supply a declarative build plan. Default: empty (use newWidget()).
    virtual std::vector<PlanNode> getPlan() { return {}; }
    /**
