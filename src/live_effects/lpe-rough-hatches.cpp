...
std::vector<PlanNode> LPERoughHatches::getPlan() {
    return {
        row({p(&dist_rdm), p(&growth)}),
        p(&do_bend),
        // edge variations: top/bottom pairs
        row({p(&top_edge_variation), p(&bot_edge_variation)}),
        // tangent variations: top/bottom pairs
        row({p(&top_tgt_variation), p(&bot_tgt_variation)}),
        // smoothness scales: bottom ←/→ and top ←/→
        row({p(&scale_tf), p(&scale_tb)}),
        row({p(&scale_bf), p(&scale_bb)}),
        // smoothness randomness: top/bottom pairs
        row({p(&top_smth_variation), p(&bot_smth_variation)}),
        p(&fat_output),
        // stroke widths: top/bottom pair
        row({p(&stroke_width_top), p(&stroke_width_bot)}),
        // thickness: front/back pair
        row({p(&front_thickness), p(&back_thickness)}),
    };
}
