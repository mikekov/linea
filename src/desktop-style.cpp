...
sp_desktop_set_style(Inkscape::ObjectSet *set, SPDesktop *desktop, SPCSSAttr *css, bool change, bool write_current, bool switch_style)
{
    if (write_current) {
        ...
        sp_repr_css_merge(desktop->_current, css);
        ...
        desktop->fireDesktopStyleChanged();
    }
...
{
    if (!desktop || !desktop->_current) return std::nullopt;

    gchar const *property = sp_repr_css_property(desktop->_current,
                                                 is_fill ? "fill" : "stroke",
                                                 "#000");
...
std::tuple<Linea::PaintProp, Linea::PaintProp, SPIPaintOrder> sp_desktop_get_current_style_paints(SPDesktop* desktop, UI::Tools::ToolBase* tool) {
    Linea::PaintProp fill;
    Linea::PaintProp stroke;

    if (!desktop || !desktop->_current || !desktop->getDocument()) {
        return {fill, stroke, SPIPaintOrder{}};
    }

    SPCSSAttr* css = nullptr;
    if (tool) {
        if (auto toolData = getToolData(tool->get_name())) {
            if (toolData->ownStyle) {
                css = desktop->getCurrentOrToolStyle(tool->getPrefsPath(), false);
            }
        }
    }

    SPStyle style(desktop->getDocument());
    style.mergeCSS(css ? css : desktop->_current);

    fill  = Linea::classify_paint(style.fill,  style.fill_opacity.as_double());
    stroke = Linea::classify_paint(style.stroke, style.stroke_opacity.as_double());

    return {fill, stroke, style.paint_order};
}
...
void sp_desktop_apply_color(SPDesktop* desktop, const std::optional<Colors::Color>& maybeColor, bool stroke, UI::Tools::ToolBase* tool) {
    if (!desktop) return;
    ...
    else { //if (const auto& color = *maybeColor) {
        sp_repr_css_set_property_string(css.get(), attr_name, maybeColor->toString());
    ...
    if (tool) {
        Inkscape::Preferences *prefs = Inkscape::Preferences::get();
        // Save the color to the style source this tool actually reads
        // (which may be the tool's own style, the desktop current style,
        // or a per-object-type style depending on usecurrent).
        prefs->mergeStyle(desktop->getCurrentOrToolStylePath(tool->getPrefsPath()), css.get());
    ...
    DocumentUndo::done(desktop->getDocument(), *description, "");
}
