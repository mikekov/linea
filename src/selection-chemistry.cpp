...
void ObjectSet::deleteItems(bool skip_undo)
{
    ...
    if (SPDesktop *dt = desktop()) {
         ...
         */
        dt->setTool(dt->getTool()->get_name());
    }
...
Geom::Rect sp_bbox_to_xywh(const Geom::OptRect& bbox, Inkscape::Selection* selection,
                             SPDesktop* desktop, const Inkscape::Util::Unit* unit) {
    if (!bbox || !selection || !desktop || !unit) return Geom::Rect();

    auto width = bbox->width();
    auto height = bbox->height();
    auto x = bbox->left() + width * selection->anchor.x();
    auto y = bbox->top() + height * selection->anchor.y();
...
Geom::Rect sp_selection_get_xywh(SPDesktop* desktop, const Inkscape::Util::Unit* unit, bool use_visual_box) {
    ...
    return sp_bbox_to_xywh(bbox, sel, desktop, unit);
}
