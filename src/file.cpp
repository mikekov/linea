 ...
 */
static bool
file_save(LineaWindow& parentWindow,
          SPDocument *doc,
...
bool sp_file_save_dialog(LineaWindow& parentWindow, SPDocument *doc, Inkscape::Extension::FileSaveMethod save_method)
{
    ...
    if (!found) {
     ...
     if (file_save(parentWindow, doc, file, extension, true, !is_copy, save_method)) {
        if (!is_copy) {
            auto docname = doc->getDocumentName();
 ...
 */
bool
sp_file_save_document(LineaWindow& parentWindow, SPDocument *doc)
{
    if (auto path = doc->getDocumentFilename()) {
        ...
        if (file_save(parentWindow, doc, file, Inkscape::Extension::db.get(ext), false, true, Inkscape::Extension::FILE_SAVE_METHOD_SAVE_AS)) {
            auto docname = doc->getDocumentName();
            Linea::IO::addInkscapeRecentSvg(path, docname ? docname : "");
            return true;
        }
    }
 ...
 */
bool
sp_file_save(LineaWindow& parentWindow, gpointer /*object*/, gpointer /*data*/)
{
 ...
 */
bool
sp_file_save_as(LineaWindow& parentWindow, gpointer /*object*/, gpointer /*data*/)
{
    if (!SP_ACTIVE_DOCUMENT) {
        return false;
    }
 ...
 */
bool
sp_file_save_a_copy(LineaWindow& parentWindow, gpointer /*object*/, gpointer /*data*/)
{
    if (!SP_ACTIVE_DOCUMENT) {
        return false;
    }
 ...
 */
bool
sp_file_save_template(LineaWindow& parentWindow, Glib::ustring name,
                      ...
                      std::optional<Geom::Point> drop_pos)
{
    ...
    // Snap the imported object's top-left corner to integer pixel coordinates.
    // TODO: verify that it works well with different units
    auto const top_left = bbox_doc.min() + transform.translation();
    transform.setTranslation(Geom::Point(top_left.round()) - bbox_doc.min());
