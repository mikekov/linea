...
void Selection::_schedule_modified(SPObject */*obj*/, guint flags) {
    if (!_modified_timer) {
        _modified_timer = new QTimer(this);
        _modified_timer->setSingleShot(true);
        QObject::connect(_modified_timer, &QTimer::timeout, this, [this]() {
            auto flags = _flags;
            _flags = 0;
            _emitModified(flags);
        });
    }
    _modified_timer->start(0);
...
{
    if (_modified_block_count > 0) {
        return;  // Blocked during drag operations
    }
    // printf("emit modified selectiton: %d\n", flags);
    _modified_signal.emit(this, flags);

    if (!_desktop || !_document || isEmpty()) {
        return;
    }
