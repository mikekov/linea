...
// default colors
static const auto GRID_DEFAULT_MAJOR_COLOR = Inkscape::Colors::Color{0x6aabcbff}.withOpacity(0.36);
static const auto GRID_DEFAULT_MINOR_COLOR = Inkscape::Colors::Color{0x6aabcbff}.withOpacity(0.18);
...
void SPGrid::set(SPAttr key, const gchar* value)
{
    switch (key) {
        ...
        case SPAttr::SNAP_TO_VISIBLE_ONLY:
            ...
            requestDisplayUpdate(SP_OBJECT_MODIFIED_FLAG);
            break;
        case SPAttr::CLIP_TO_PAGE:
            _cliptopage.read(value);
            requestDisplayUpdate(SP_OBJECT_MODIFIED_FLAG);
            break;
        default:
    ...
    if (_legacy) {
        ...
        switch (type) {
            ...
            case GridType::AXONOMETRIC:
                ...
                fix(SPAttr::SPACINGX, "1"); // not used by axonometric grid, but required for it to render
                break;
...
void SPGrid::setPrefValues()
{
    ...
    setMajorColor(prefs->getColor(prefix + "/empcolor", /*modular ? "#0047cb4d" :*/ GRID_DEFAULT_MAJOR_COLOR.toString()));
    setMinorColor(prefs->getColor(prefix + "/color", /*modular ? "#0047cb26" :*/ GRID_DEFAULT_MINOR_COLOR.toString()));
...
void SPGrid::update(SPCtx *ctx, unsigned int flags)
{
    ...
    // Build the page rect from the union of all real pages, falling back to the
    // document preferred bounds when no pages are managed by the PageManager.
    Geom::OptRect page_rect;
    if (_cliptopage) {
        if (auto page = document->getPageManager().getSelected()) {
            page_rect = page->getDesktopRect();
        }
        else {
            for (auto page : document->getPageManager().getPages()) {
                page_rect.unionWith(page->getDesktopRect());
            }
        }
        if (!page_rect) {
            if (auto bounds = document->preferredBounds()) {
                page_rect = *bounds;
            }
        }
    }
...
void SPGrid::setClipToPage(bool v)
{
    getRepr()->setAttributeBoolean("cliptopage", v);
