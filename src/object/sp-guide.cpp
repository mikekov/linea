...
void SPGuide::set_color(const unsigned r, const unsigned g, const unsigned b, unsigned a, bool const commit)
{
    ...
    if (commit) {
        char buf[16];
        // preserve color with alpha
        snprintf(buf, sizeof(buf), "#%02x%02x%02x%02x", r, g, b, a);
        // std::ostringstream os;
        // os << "rgb(" << r << "," << g << "," << b << ")";
        //XML Tree being used directly while it shouldn't be
        setAttribute("inkscape:color", buf);
    }
}
...
Inkscape::CanvasItemGuideLine* SPGuide::findGuide(Inkscape::UI::Widget::Canvas* canvas) const {
    for (auto& view : views) {
        if (view->get_canvas() == canvas) {
            return view.get();
        }
    }
    return nullptr;
