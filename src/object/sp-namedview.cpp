...
static auto const default_desk_color = Inkscape::Colors::Color{0xe1e1e1, false};
static auto const default_guide_color = Inkscape::Colors::Color{0x6aabcbff, false};
static auto const default_guide_hi_color = Inkscape::Colors::Color{0xfd7e14ff, false};

Inkscape::Colors::Color SPNamedView::getDeskColor() const
{
    auto& bgn = document->getPageManager().getBackgroundColor();
    auto color = _desk_color.value_or(default_desk_color);
    color.setOpacity(bgn.hasOpacity() ? bgn.getOpacity() : 1.0);
    return color;
}
...
bool SPNamedView::desk_checkerboard() const {
    auto& bgn = document->getPageManager().getBackgroundColor();
    return bgn.hasOpacity() && bgn.getOpacity() < 1.0;
}
...
void sp_namedview_window_from_document(SPDesktop *desktop)
{
    ...
    if (window_geometry == PREFS_WINDOW_GEOMETRY_LAST) {
        ...
        if (pw>0 && ph>0) {
            ...
            auto monitor_geometry = Linea::UI::get_monitor_geometry_at_point(px, py);
            pw = std::min(pw, monitor_geometry.width());
            ph = std::min(ph, monitor_geometry.height());
        ...
        if (window_geometry == PREFS_WINDOW_GEOMETRY_FILE && !new_document) {
            auto monitor_geometry = Linea::UI::get_monitor_geometry_at_point(nv->window_x, nv->window_y);
            w = std::min(monitor_geometry.width(), nv->window_width);
            h = std::min(monitor_geometry.height(), nv->window_height);
            move_to_screen = true;
        } else if (default_size == PREFS_WINDOW_SIZE_LARGE) {
            auto monitor_geometry = Linea::UI::get_monitor_geometry_at_surface(win->get_surface());
            w = std::max<int>(0.75 * monitor_geometry.width(), MIN_WINDOW_SIZE);
            h = std::max<int>(0.75 * monitor_geometry.height(), MIN_WINDOW_SIZE);
...
void sp_namedview_update_layers_from_document (SPDesktop *desktop)
{
    ...
    // don't use that object unless it's an actual layer
    if (layer) {
        layer = desktop->layerManager().asLayer(layer);
    }
    // if that didn't work out, look for the topmost layer
    if (!layer) {
        for (auto& iter: document->getRoot()->children) {
            if (auto* grp = desktop->layerManager().asLayer(&iter)) {
                layer = grp;
            }
        }
    }
...
void sp_namedview_document_from_window(SPDesktop *desktop)
{
    ...
    if (auto layer = desktop->layerManager().currentLayer()) {
        view->setAttribute("inkscape:current-layer", layer->getId());
    }
}
...
bool SPNamedView::getShowGrids() const
{
...
void SPNamedView::change_color(SPAttr color_key, const Inkscape::Colors::Color& color) {
    // save color with opacity; separate opacity field is not used
    getRepr()->setAttribute(sp_attribute_name(color_key), color.toString(true));
}

void SPNamedView::change_color(SPAttr color_key, SPAttr opacity_key, Inkscape::Colors::Color const &color) {
    if (color.hasOpacity()) {
        getRepr()->setAttributeCssDouble(sp_attribute_name(opacity_key), color.getOpacity(), Inkscape::Util::get_opacity_default_precision());
    }
