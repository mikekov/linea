...
/// Type returned by ObjectSet::objects() — the random_access index of the
/// multi_index container. Exposed so callers can iterate the selection
/// without materializing a copy.
using RandomAccessIndex = MultiIndexContainer::index<random_access>::type;
...
public:
    ...
    /** Returns the list of selected objects (const version). */
    ...
    auto objects_of_type() {
        return objects()
            | std::views::filter([](auto object) { return is<T>(object) && !is<SPRoot>(object); })
            | std::views::transform(cast_unsafe<T>);
    }

    template <typename T>
    requires std::is_base_of_v<SPObject, T>
    auto objects_of_type() const {
        return objects()
            | std::views::filter([](auto object) { return is<T>(object) && !is<SPRoot>(object); })
            | std::views::transform(cast_unsafe<T>);
    }
    ...
    /** Returns a range of selected SPItems (const version). */
    auto items() const {
        return objects_of_type<SPItem>();
    }
