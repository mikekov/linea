...
void SPMask::release()
{
    ...
    // Hide child views before removing the mask view, so its DrawingItem does not delete
    // child DrawingItems that are still owned by the mask's child SPItems.
    while (!views.empty()) {
        auto key = views.back().key;
        hide(key);
    }
