...
static void gr_knot_moved_handler(SPKnot *knot, Geom::Point const &ppointer, guint state, gpointer data)
{
    ...
    } else if (!(state & INK_CONTROL_MASK)) {
    ...
    if (!((state & INK_SHIFT_MASK) || (state & INK_CONTROL_MASK))) {
        m.setup(desktop);
    ...
    bool scale_radial = (state & INK_CONTROL_MASK) && (state & INK_SHIFT_MASK);
...
static void gr_knot_ungrabbed_handler(SPKnot *knot, unsigned int state, gpointer data)
{
    ...
    if ((state & INK_CONTROL_MASK) && (state & INK_SHIFT_MASK)) {
...
static void gr_knot_clicked_handler(SPKnot */*knot*/, guint state, gpointer data)
{
    ...
    if ( (state & INK_CONTROL_MASK) && (state & INK_ALT_MASK ) ) {
...
{
    selected.clear();
    // delete old draggers, but preserve any whose knot is currently grabbed
    // (the gradient tool grabs a knot right after updateDraggers; a late
    // sel_modified signal can trigger a second updateDraggers that would
    // destroy the grabbed knot and kill the drag)
    std::vector<GrDragger*> kept;
    for (auto dragger : this->draggers) {
        if (dragger->knot && dragger->knot->grabbed) {
            kept.push_back(dragger);
        } else {
            delete dragger;
        }
    }
    this->draggers.clear();
    // put kept (grabbed) draggers back
    for (auto dragger : kept) {
        this->draggers.push_back(dragger);
    }
 ...
 * @param event Event with type INK_KEY_PRESS
...
bool GrDrag::key_press_handler(Inkscape::KeyPressEvent const &event)
{
    ...
    switch (keyval) {
        case INK_KEY_Left: // move handle left
        case INK_KEY_KP_Left:
        case INK_KEY_KP_4:
            x_dir = -1;
            break;

        case INK_KEY_Up: // move handle up
        case INK_KEY_KP_Up:
        case INK_KEY_KP_8:
            y_dir = 1;
            break;

        case INK_KEY_Right: // move handle right
        case INK_KEY_KP_Right:
        case INK_KEY_KP_6:
            x_dir = 1;
            break;

        case INK_KEY_Down: // move handle down
