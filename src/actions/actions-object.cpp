...
void object_trace(QString const &value, LineaApplication *app)
{
    ...
    Glib::ustring const str = value.toStdString();
    std::vector<Glib::ustring> const settings = Glib::Regex::split_simple(",", str);
    if (settings.size() != 7) {
...
void
object_get_attribute(QString const &value, LineaApplication *app)
{
    ...
    Glib::ustring const attribute = value.toStdString();
...
void
object_get_property(QString const &value, LineaApplication *app)
{
    SPDocument* document = nullptr;
    Inkscape::Selection* selection = nullptr;
    if (!get_document_and_selection(app, &document, &selection)) {
...
void
object_remove_attribute(QString const &value, LineaApplication *app)
{
...
void
object_remove_property(QString const &value, LineaApplication *app)
{
    Inkscape::Selection *selection = app->get_active_selection();
    if (!selection || selection->isEmpty()) {
    ...
    Glib::ustring const property = value.toStdString();
...
void
object_set_attribute(QString const &value, LineaApplication *app)
{
    Glib::ustring const argument = value.toStdString();
...
// No sanity checking is done... should probably add.
void
object_set_property(QString const &value, LineaApplication *app)
{
    Glib::ustring const s = value.toStdString();
...
void
object_unlink_clones(LineaApplication *app)
{
    auto selection = app->get_active_selection();
    if (!selection) {
        return;
...
void
object_clip_set(LineaApplication *app)
{
    Inkscape::Selection *selection = app->get_active_selection();
    if (!selection) {
        return;
...
void
object_clip_set_inverse(LineaApplication *app)
{
    Inkscape::Selection *selection = app->get_active_selection();
    if (!selection) {
        return;
...
void
object_clip_release(LineaApplication *app)
{
    Inkscape::Selection *selection = app->get_active_selection();
    if (!selection) {
        return;
...
void
object_clip_set_group(LineaApplication *app)
{
    Inkscape::Selection *selection = app->get_active_selection();
    if (!selection) {
        return;
...
void
object_mask_set(LineaApplication *app)
{
    Inkscape::Selection *selection = app->get_active_selection();
    if (!selection) {
        return;
...
void
object_mask_set_inverse(LineaApplication *app)
{
    Inkscape::Selection *selection = app->get_active_selection();
    if (!selection) {
        return;
...
void
object_mask_release(LineaApplication *app)
{
    Inkscape::Selection *selection = app->get_active_selection();
    if (!selection) {
        return;
...
void
object_rotate_90_cw(LineaApplication *app)
{
    Inkscape::Selection *selection = app->get_active_selection();
    if (!selection) {
        return;
...
void
object_rotate_90_ccw(LineaApplication *app)
{
    Inkscape::Selection *selection = app->get_active_selection();
    if (!selection) {
        return;
...
void
object_flip_horizontal(LineaApplication *app)
{
    Inkscape::Selection *selection = app->get_active_selection();
    if (!selection) {
        return;
...
void
object_flip_vertical(LineaApplication *app)
{
    Inkscape::Selection *selection = app->get_active_selection();
    if (!selection) {
        return;
...
void
object_star_turn_upright(LineaApplication *app)
{
    auto selection = app->get_active_selection();
    if (!selection || selection->isEmpty()) {
...
void
object_to_path(LineaApplication *app)
{
    auto selection = app->get_active_selection();
    if (!selection) {
        return;
...
void
object_add_corners_lpe(LineaApplication *app) {
    auto selection = app->get_active_selection();
    if (!selection) {
        return;
    }
...
void
object_stroke_to_path(LineaApplication *app)
{
    auto selection = app->get_active_selection();
    if (!selection) {
        return;
...
static auto object_action_defs = std::to_array<ApplicationActionDef>({
    // clang-format off
    {"object-set-attribute",       N_("Set Attribute"),           SECTION, N_("Set or update an attribute of selected objects"),
        [](auto app){ object_set_attribute(QString(), app); }},
    {"object-set-property",        N_("Set Property"),            SECTION, N_("Set or update a property on selected objects"),
        [](auto app){ object_set_property(QString(), app); }},
    {"object-get-attribute",       N_("Get Attribute"),           SECTION, N_("Get the value of an attribute of selected objects"),
        [](auto app){ object_get_attribute(QString(), app); }},
    {"object-get-property",        N_("Get Property"),            SECTION, N_("Get the value of a property on selected objects"),
        [](auto app){ object_get_property(QString(), app); }},
    {"object-remove-attribute",    N_("Remove Attribute"),        SECTION, N_("Remove an attribute on selected objects"),
        [](auto app){ object_remove_attribute(QString(), app); }},
    {"object-remove-property",     N_("Remove Property"),         SECTION, N_("Remove a property on selected objects"),
        [](auto app){ object_remove_property(QString(), app); }},
    {"object-trace",               N_("Trace Bitmap"),            SECTION, N_("Trace selected bitmap"),
        [](auto app){ object_trace(QString(), app); }},

    {"object-unlink-clones",       N_("Unlink Clones"),           SECTION, N_("Unlink clones and symbols"),
        object_unlink_clones},
    ...
    {"object-set-clip-group",      N_("Object Clip Set Group"),   SECTION, N_("Create a self-clipping group to which objects (not contributing to the clip-path) can be added"),
        object_clip_set_group},
    {"object-set-mask",            N_("Object Mask Set"),         SECTION, N_("Apply mask to selection (using the topmost object as mask)"),
        object_mask_set},
    {"object-set-inverse-mask",    N_("Object Mask Set Inverse"), SECTION, N_("Apply inverse mask to selection (Power Mask LPE)"),          object_mask_set_inverse},
    {"object-release-mask",        N_("Object Mask Release"),     SECTION, N_("Remove mask from selection"),                                object_mask_release},

    // Deprecated, see app.transform-rotate(90)
    {"object-rotate-90-cw",        N_("Object Rotate 90°"),       SECTION, N_("Rotate selection 90° clockwise"),                            object_rotate_90_cw},
    {"object-rotate-90-ccw",       N_("Object Rotate -90°"),      SECTION, N_("Rotate selection 90° counter-clockwise"),                    object_rotate_90_ccw},
    {"object-flip-horizontal",     N_("Object Flip Horizontal"),  SECTION, N_("Flip selected objects horizontally"),                        object_flip_horizontal},
    {"object-flip-vertical",       N_("Object Flip Vertical"),    SECTION, N_("Flip selected objects vertically"),                          object_flip_vertical},
    {"object-star-turn-upright",   N_("Turn Stars/Polygons Upright"), SECTION, N_("Turn stars and polygons upright"),                       object_star_turn_upright}
    // clang-format on
});
