...
#if 0
    g_warning("get_snapping_preferences: global=%s bbox=%s node=%s others=%s",
              preferences.getSnapEnabledGlobally() ? "true" : "false",
              preferences.isTargetSnappable(SNAPTARGET_BBOX_CATEGORY) ? "true" : "false",
              preferences.isTargetSnappable(SNAPTARGET_NODE_CATEGORY) ? "true" : "false",
              preferences.isTargetSnappable(SNAPTARGET_OTHERS_CATEGORY) ? "true" : "false");
    g_warning("get_snapping_preferences: grid=%s guide=%s alignment=%s distribution=%s",
              preferences.isTargetSnappable(SNAPTARGET_GRID) ? "true" : "false",
              preferences.isTargetSnappable(SNAPTARGET_GUIDE) ? "true" : "false",
              preferences.isTargetSnappable(SNAPTARGET_ALIGNMENT_CATEGORY) ? "true" : "false",
              preferences.isTargetSnappable(SNAPTARGET_DISTRIBUTION_CATEGORY) ? "true" : "false");
#endif
    return preferences;
}
...
void transition_to_simple_snapping() {
    ...
    //     if (Gio::ActionMap* map = dt->getLineaWindow()) {
...
const SnapEntry snapQtTable[] = {
    // clang-format off
    {{"snap-global-toggle",   N_("Toggle Snapping"),               N_("Toggle snapping on/off"),       "snap", nullptr},
     []() {
         auto& pref = get_snapping_preferences();
         pref.setSnapEnabledGlobally(!pref.getSnapEnabledGlobally());
         store_snapping_action(global_toggle, pref.getSnapEnabledGlobally());
     },
     []() { return get_snapping_preferences().getSnapEnabledGlobally(); }},
    {{"simple-snap-bbox",      N_("Snap Bounding Box"), N_("Toggle snapping to bounding boxes"), "snap-bounding-box", N_("Snap Bounding Box Off")},
     []() { set_simple_snap(SimpleSnap::BBox,      !get_snapping_preferences().get_simple_snap(SimpleSnap::BBox)); },
     []() { return get_snapping_preferences().get_simple_snap(SimpleSnap::BBox); }},
    {{"simple-snap-nodes",     N_("Snap Nodes"),        N_("Toggle snapping to nodes"),          "snap-nodes", N_("Snap Nodes Off")},
     []() { set_simple_snap(SimpleSnap::Nodes,     !get_snapping_preferences().get_simple_snap(SimpleSnap::Nodes)); },
     []() { return get_snapping_preferences().get_simple_snap(SimpleSnap::Nodes); }},
    {{"simple-snap-alignment", N_("Snap Alignment"),    N_("Toggle alignment snapping"),         "snap-alignment"},
     []() { set_simple_snap(SimpleSnap::Alignment, !get_snapping_preferences().get_simple_snap(SimpleSnap::Alignment)); },
     []() { return get_snapping_preferences().get_simple_snap(SimpleSnap::Alignment); }},
    // clang-format on
};
...
void add_actions_canvas_snapping(LineaWindow* win) {
    ...
    registry.registerGroup(snapActionGroup);

    for (auto& e : snapQtTable) {
        QAction* a;
        if (e.state_fn) {
            ...
            a = registry.createAction(e.meta, e.fn);
        }
        win->addAction(a);
    }
}
