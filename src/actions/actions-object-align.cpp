...
/*
 * Qt Actions for aligning and distributing objects without GUI.
 *
...
public:
    ...
    bool operator< (const Baseline &b) const
    {
        return _base[_orientation] < b._base[b._orientation];
    }
...
object_remove_overlaps(QString const &value, Inkscape::Selection *selection)
{
    ...
    if (parts.size() >= 1) {
        hgap = parts[0].toDouble();
    }
    if (parts.size() >= 2) {
        vgap = parts[1].toDouble();
    }
...
void add_actions_object_align(LineaApplication* app) {
    ...
    auto on_canvas_action = registry.createBoolAction(
        ...
        []() -> bool {
            return Inkscape::Preferences::get()->getBool("/dialogs/align/oncanvas");
        },
        on_canvas
    );
    app->get_active_window()->addAction(on_canvas_action);
}
