...
SPStyle::cascade( SPStyle const *const parent ) {
    ...
    for(std::vector<SPIBase*>::size_type i = 0; i != _properties.size(); ++i) {
        ...
        if (!_properties[i]->set && parent->_properties[i]->css_var) {
            _properties[i]->css_var = parent->_properties[i]->css_var;
        }
    }
}
...
SPStyle::_mergeDecl(  CRDeclaration const *const decl, SPStyleSrc const &source ) {
    ...
    if (prop_idx != SPAttr::INVALID) {
        ...
        if (!isSet(prop_idx) || decl->important) {
            ...
            // Resolve var(...) against global CSS custom properties if available.
            Glib::ustring resolved = str_value;
            const bool is_var = (document && strstr(str_value, "var(") != nullptr);
            if (is_var) {
                resolved = document->resolveCssVariableValue(str_value);
            }
            ...
            os << resolved.c_str() << important;

            readIfUnset(prop_idx, os.str().c_str(), source);

            if (is_var) {
                if (auto p = _prop_helper.get(this, prop_idx)) {
                    p->css_var = std::make_shared<Glib::ustring>(str_value);
                    if (!p->set) {
                        p->set = true;
                        p->style_src = source;
                        if (decl->important) {
        ...
        if (g_str_has_prefix(key, "--")) {
            // Custom properties are collected at the document level.
            // Only minimal css variable support is available for now.
