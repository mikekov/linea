...
#include "qt/ui/dialog/crash-dialog.h"
// #include "ui/themes.h"
    ...
    _use_gui(use_gui)
{
    ...
    if (use_gui) {
        ...
        // Inkscape::UI::Widget::register_all(); // QT TODO: GTK widget registry not available
    }
...
Application::crash_handler (int /*signum*/)
{
    ...
    fprintf (stderr, "Emergency save completed. Linea will close now.\n");
    fprintf (stderr, "If you can reproduce this crash, please file a bug at https://codeberg.org/mkov/linea/issues\n");
    ...
    *(b + pos) = '\0';
    ...
    auto stacktrace = boost::stacktrace::to_string(boost::stacktrace::stacktrace());
    stacktrace += "\nSystem info\n" + debug_info();

    if (exists() && instance().use_gui()) {
        try {
            auto autosaves = QString::fromUtf8(b);
            auto details = QString::fromStdString(stacktrace);
            Linea::UI::CrashDialog dialog(autosaves, details);
            dialog.exec();
        } catch (const std::exception& ex) {
            g_message("Crash dialog failed: %s", ex.what());
        }
    }
