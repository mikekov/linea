...
std::set<SPBlendMode> const FilterBlend::_valid_modes {
    // clang-format off
...
FilterBlend::~FilterBlend() = default;
...
bool FilterBlend::can_handle_affine(Geom::Affine const &) const
{
    // blend is a per-pixel primitive and is immutable under transformations
    return true;
}

double FilterBlend::complexity(Geom::Affine const &) const
{
    return 1.1;
}

bool FilterBlend::uses_background() const
{
