...
void DrawingText::decorateStyle(DrawingContext &dc, double vextent, double xphase, Geom::Point const &p1, Geom::Point const &p2, double thickness) const
{
    ...
    */
        ...
        for (double gcx = first_global; gcx < global_end; gcx += dot_spacing) {
            ...
            // Start a new subpath for each dot so cairo_arc doesn't insert a
            // connecting line from the previous arc's end to this one's start.
            // Without this, all dots form one subpath and cairo_fill() implicitly
            // closes it, drawing a stray line from the last dot back to the first.
