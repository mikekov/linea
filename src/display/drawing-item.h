...
protected:
    ...
    // Bounding box as of the last time it was invalidated for rendering (e.g. before a transform
    // change takes effect), pending being unioned into the next _markForRendering() dirty rect.
    // This ensures fast/large moves invalidate the whole swept path (the bounding rect of the old
    // and new positions), not just the two endpoint boxes with a potential gap between them.
    Geom::OptIntRect _pending_dirty;
