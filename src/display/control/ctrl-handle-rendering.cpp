...
// bottom/left corner
void draw_corner_bl(Cairo::Context& ctx, double size, int pass) {
    double const t = size * 0.15;
    double const h = size / 2.0;
    if (pass == 1) {
        ctx.move_to(h, 0);
        ctx.line_to(h, h);
        ctx.line_to(size, h);
    ...
    if (pass == 2) {
        ...
        ctx.line_to(t, h);
        ctx.move_to(h, size);
        ctx.line_to(h, size - t);
    }
}

// top/right corner
void draw_corner_tr(Cairo::Context& ctx, double size, int pass) {
    double const t = size * 0.15;
    double const h = size / 2.0;
    if (pass == 1) {
        ctx.move_to(h, size);
...
void draw_cairo_path(CanvasItemCtrlShape shape, Cairo::Context &cr, double size, double width, double angle)
{
    switch (shape) {
        ...
        case CANVAS_ITEM_CTRL_SHAPE_CORNER_BL:
            draw_corner_bl(cr, size, 1);
            break;

        case CANVAS_ITEM_CTRL_SHAPE_CORNER_TR:
            draw_corner_tr(cr, size, 1);
            break;
...
bool draw_cairo_path_2nd_pass(CanvasItemCtrlShape shape, Cairo::Context &cr, double size, double width, double angle) {
    switch (shape) {
        case CANVAS_ITEM_CTRL_SHAPE_CORNER_BL:
            draw_corner_bl(cr, size, 2);
            return true;

        case CANVAS_ITEM_CTRL_SHAPE_CORNER_TR:
            draw_corner_tr(cr, size, 2);
            return true;
    }
...
std::shared_ptr<Cairo::ImageSurface const> draw_uncached(RenderParams const &p)
{
    ...
    bool needs_2_passes = draw_cairo_path_2nd_pass(p.shape, cr, size, total_size - effective_outline, p.angle);
    ...
    if (needs_2_passes) {
        cr.begin_new_path();
        draw_cairo_path_2nd_pass(p.shape, cr, size, total_size - effective_outline, p.angle);

        auto color = Colors::Color(p.stroke);
