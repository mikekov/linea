...
void CanvasItemGrid::set_page_rect(Geom::OptRect const &rect)
{
    defer([=, this] {
        if (_page_rect == rect) {
            return;
...
void CanvasItemGrid::_clip_to_page(Inkscape::CanvasItemBuffer const &buf) const {
    if (!_page_rect) return;

    // _page_rect is in document coordinates; transform each corner to the same absolute
    // screen coordinates used for the grid geometry (ow, sw, etc.), which is also the space
    // buf.cr is in after the -buf.rect.left()/top() translation done by the caller.
    auto const page = *_page_rect;
    auto const page_corner = [&](unsigned index) { return page.corner(index) * affine(); };
    auto const first_corner = page_corner(0);
    buf.cr->move_to(first_corner.x(), first_corner.y());
    for (unsigned index = 1; index < 4; ++index) {
        auto const corner = page_corner(index);
