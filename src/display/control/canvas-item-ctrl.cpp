...
void CanvasItemCtrl::_set_size(float size)
{
    defer([=, this] {
...
void CanvasItemCtrl::set_size_via_index(int size_index)
{
    ...
    auto size = std::clamp(size_index + static_cast<int>(_rel_size), MIN_INDEX, MAX_INDEX) / 2.0f;
    _set_size(size);
}
...
// printf("new bounds for %p %d: x: %f\n", this, _anchor, _bounds->min().x());
