...
void CanvasItem::unlink()
{
    defer([=, this] {
        ...
        request_redraw();
// std::cout << "DEBUG: request_redraw() called from unlink" << std::endl;
        // Remove from parent.
        if (_parent) {
// std::cout << "DEBUG: erase() called from unlink" << std::endl;
...
// Grab all events!
void CanvasItem::grab(EventMask event_mask, QCursor const &cursor)
{
