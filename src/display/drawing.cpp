...
void Drawing::setCacheLimit(Geom::OptIntRect const &rect)
{
    defer([=, this] {
        _cache_limit = rect;
        if (rect && _canvas_scale != 1.0) {
            _cache_limit = (Geom::Rect(*rect) * Geom::Scale(_canvas_scale)).roundOutwards();
        }
...
{
    auto const scaled = (Geom::Rect(area) * Geom::Scale(_canvas_scale)).roundOutwards();
    auto surface = Cairo::ImageSurface::create(Cairo::Surface::Format::ARGB32, scaled.width(), scaled.height());
    auto dc = Inkscape::DrawingContext(surface->cobj(), scaled.min());
    render(dc, scaled);
