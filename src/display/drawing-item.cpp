...
// --- TEMP DEBUG INSTRUMENTATION (see matching blocks in canvas.cpp / optglarea.cpp) --------
// Enabled by LINEA_DEBUG_TILES=1. Separate translation unit, duplicated minimally here.
// TODO: remove once the rendering-glitch investigation is complete.
namespace {
namespace debugtiles {
inline bool enabled()
{
    static bool const e = std::getenv("LINEA_DEBUG_TILES") != nullptr;
    return e;
}
inline std::ofstream &stream()
{
    static std::ofstream f = [] {
        std::ofstream s("/tmp/linea_canvas_debug.log", std::ios::app);
...
void log(Args &&...args)
{
    ...
    f << "[" << std::fixed << std::setprecision(4) << now << "] ";
    (f << ... << args);
    f << std::endl;
}
} // namespace debugtiles
...
void DrawingItem::setTransform(Geom::Affine const &transform)
{
    defer([=, this] {
        ...
        _markForRendering();
        // Remember the pre-transform-change bounding box so that once update() recomputes the
        // new one and invalidates it, the two get unioned into a single rectangle covering the
        // whole swept path (see _markForRendering()), instead of leaving a potential gap between
        // the old and new positions uninvalidated (visible as a stale "trail" during fast drags).
        _pending_dirty.unionWith(_drawbox);
...
void DrawingItem::_markForRendering()
{
    ...
    // Union in any not-yet-flushed prior dirty rect (e.g. the bounding box just before a
    // transform change took effect). This ensures the bounding rectangle of the old and new
    // positions is invalidated as a whole, covering the entire swept path between them - not
    // just the two endpoint boxes, which can leave an uninvalidated gap between them when a
    // single move covers a large distance (e.g. during fast dragging).
    if (_pending_dirty) {
        dirty.unionWith(_pending_dirty);
        _pending_dirty = {};
    }

    if (!dirty) return;

    debugtiles::log("_markForRendering item=", (void const *)this,
                     " dirty=", dirty->left(), ",", dirty->top(), ",", dirty->right(), ",", dirty->bottom());
