 ...
 * Allows easy interoperation with QImage and Cairo. */
class Pixbuf {
public:
    enum PixelFormat {
        PF_CAIRO = 1,
  ...
  private:
    void _createSurface();
    void _forceAlpha();
    void _setMimeData(const guchar *data, gsize len, Glib::ustring const &format);

    QImage _image;
    cairo_surface_t *_surface = nullptr;
    std::vector<guchar> _mime_data;
    std::string _mime_type;
    time_t _mod_time = 0;
    std::string _path;
    PixelFormat _pixel_format = PF_RGBA8888;
};
...
void convert_pixels_rgba_to_argb32(guchar *data, int w, int h, int rs);
void convert_pixels_argb32_to_rgba(guchar *data, int w, int h, int rs, guint32 bgcolor=0);
