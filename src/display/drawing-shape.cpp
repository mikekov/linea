...
unsigned DrawingShape::_updateItem(Geom::IntRect const &area, UpdateContext const &ctx, unsigned flags, unsigned reset)
{
    ...
    auto calc_curve_bbox = [&, this] () -> Geom::OptIntRect {
        ...
        // changed to "fast" bounds to speed up zoom gesture for complex documents
        auto rect = bounds_fast_transformed(*_curve, ctx.ctm); //bounds_exact_transformed(*_curve, ctx.ctm);
        if (!rect) {
            return {};
        }
...
DrawingItem *DrawingShape::_pickItem(Geom::Point const &p, double delta, unsigned flags)
{
    ...
    if (_drawing.getCanvasItemDrawing()) {
        ...
        viewbox *= Geom::Scale(_drawing.canvasScale());
        viewbox.expandBy (width);
        pathv_matrix_point_bbox_wind_distance(*_curve, _ctm, p, nullptr, needfill? &wind : nullptr, &dist, 0.5 * _drawing.canvasScale(), &viewbox);
    } else {
