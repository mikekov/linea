...
void Pixbuf::_createSurface()
{
    ...
    _surface = cairo_image_surface_create_for_data(
        _image.bits(), CAIRO_FORMAT_ARGB32, _image.width(), _image.height(), _image.bytesPerLine());
}

/** Create a new Pixbuf with the image cropped to the given area. */
Pixbuf *Pixbuf::cropTo(const Geom::IntRect &area) const
{
    auto result = std::make_unique<Pixbuf>(_image.copy(area.left(), area.top(), area.width(), area.height()));
    result->ensurePixelFormat(_pixel_format);
    result->_mime_data = _mime_data;
    result->_mime_type = _mime_type;
    result->_path = _path;
    result->_mod_time = _mod_time;
...
Pixbuf *Pixbuf::create_from_data_uri(gchar const *uri_data, double svgdpi)
{
    ...
    gsize decoded_len = 0;
    auto decoded = g_base64_decode(data, &decoded_len);
    if (!decoded) {
        return nullptr;
    }

    std::string buffer(reinterpret_cast<char const *>(decoded), decoded_len);
    g_free(decoded);
    return create_from_buffer(std::move(buffer), svgdpi, type == Base64Data::SVG ? "embedded.svg" : "");
}

Pixbuf *Pixbuf::create_from_file(std::string const &fn, double svgdpi)
{
    QFile file(QString::fromUtf8(fn.c_str()));
    if (!file.open(QIODevice::ReadOnly)) {
        return nullptr;
    }

    auto data = file.readAll();
    auto pb = create_from_buffer({data.constData(), static_cast<std::size_t>(data.size())}, svgdpi, fn);
    if (pb) {
        pb->_mod_time = QFileInfo(file).lastModified().toSecsSinceEpoch();
    }
    return pb;
}
...
Pixbuf *Pixbuf::create_from_buffer(std::string const &buffer, double svgdpi, std::string const &fn)
{
    ...
    bool is_svg = idx != std::string::npos && boost::iequals(fn.substr(idx + 1), "svg");

    if (is_svg) {
        auto svgDoc = SPDocument::createNewDocFromMem({buffer.data(), buffer.size()}, fn.c_str());
        if (!svgDoc || !svgDoc->getRoot()) {
            return nullptr;
        }

        double dpi = Linea::UI::get_effective_default_dpi();
        if (svgdpi > 0) {
            dpi = svgdpi;
        }
        auto svgWidth = svgDoc->getWidth();
        auto svgHeight = svgDoc->getHeight();
        auto width = std::min(svgWidth.value("px"), dpi * 100);
        auto height = std::min(svgHeight.value("px"), dpi * 100);
        if (width < 0 || height < 0) {
            return nullptr;
        }

        auto result = sp_generate_internal_bitmap(svgDoc.get(), Geom::Rect(0, 0, width, height), dpi);
        if (result) {
            result->_path = fn;
            result->_setMimeData(reinterpret_cast<guchar const *>(buffer.data()), buffer.size(), "svg");
        }
        return result;
    }

    QBuffer input;
    input.setData(QByteArray::fromRawData(buffer.data(), static_cast<int>(buffer.size())));
    if (!input.open(QIODevice::ReadOnly)) {
        return nullptr;
    }

    QImageReader reader(&input);
    // Qt applies embedded orientation to decoded pixels. TODO: inspect transformation() and do not
    // preserve the original MIME bytes when a transformation is present: some consumers ignore
    // EXIF orientation, so embedding the original bytes could display differently elsewhere.
    reader.setAutoTransform(true);
    auto image = reader.read();
    if (image.isNull()) {
        std::cerr << "Pixbuf::create_from_buffer: failed to load contents: " << fn << std::endl;
        return nullptr;
    }

    auto result = std::make_unique<Pixbuf>(image);
    result->_path = fn;
    auto format = reader.format().toLower();
    auto mime = format == "jpg" || format == "jpeg" ? "jpeg" : format.constData();
    result->_setMimeData(reinterpret_cast<guchar const *>(buffer.data()), buffer.size(), mime);
    return result.release();
}

QImage Pixbuf::image(bool convert_format)
{
    if (convert_format) {
        ensurePixelFormat(PF_RGBA8888);
    }
    return _image;
}

QImage Pixbuf::image() const
{
    return _image;
}
...
{
    return const_cast<Pixbuf *>(this)->getSurfaceRaw();
}
...
int Pixbuf::width() const { return _image.width(); }
int Pixbuf::height() const { return _image.height(); }
int Pixbuf::rowstride() const { return _image.bytesPerLine(); }
guchar const *Pixbuf::pixels() const { return _image.constBits(); }
guchar *Pixbuf::pixels() { return _image.bits(); }
...
bool Pixbuf::hasMimeData() const
{
    return !_mime_data.empty();
}
...
void Pixbuf::_setMimeData(const guchar *data, gsize len, Glib::ustring const &format)
{
    if (!data || !len) {
        return;
    }
    _mime_data.assign(data, data + len);
    _mime_type = format == "jpeg" || format == "jpg" ? "image/jpeg" :
                 format == "png" ? "image/png" :
                 format == "svg" || format == "svg+xml" ? "image/svg+xml" :
                 "image/" + std::string(format);
}

void Pixbuf::ensurePixelFormat(PixelFormat fmt)
{
    if (fmt == _pixel_format) {
        return;
    }
    if (_surface) {
        cairo_surface_destroy(_surface);
    ...
    _image = _image.convertToFormat(fmt == PF_CAIRO ? QImage::Format_ARGB32_Premultiplied
 ...
 */
QImage image_from_cairo_surface(cairo_surface_t *s)
{
    if (!s || cairo_surface_get_type(s) != CAIRO_SURFACE_TYPE_IMAGE) {
        return {};
    }
    cairo_surface_flush(s);
    return QImage(cairo_image_surface_get_data(s),
                  cairo_image_surface_get_width(s),
                  cairo_image_surface_get_height(s),
                  cairo_image_surface_get_stride(s),
...
/* The following two functions use "from" instead of "to", because when you write:
   val1 = argb32_from_rgba(val1);
   the name of the format is closer to the value in that format. */

guint32 argb32_from_rgba8888(guint32 c)
{
    uint32_t a;
 ...
 */
guint32 rgba8888_from_argb32(guint32 c, guint32 bgcolor)
{
 ...
 */
void
convert_pixels_rgba_to_argb32(guchar *data, int w, int h, int stride)
{
 ...
 */
void
convert_pixels_argb32_to_rgba(guchar *data, int w, int h, int stride, guint32 bgcolor)
{
    if (!data || w < 1 || h < 1 || stride < 1) {
        return;
    }
    for (size_t i = 0; i < h; ++i) {
        guint32 *px = reinterpret_cast<guint32*>(data + i*stride);
        for (size_t j = 0; j < w; ++j) {
            *px = rgba8888_from_argb32(*px, bgcolor);
            ++px;
        }
    }
}
