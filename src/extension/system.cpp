...
std::unique_ptr<SPDocument> open(Extension *key, char const *filename, bool is_importing)
{
    ...
    // Holding Shift while opening forces the import preferences dialog,
    // regardless of the "don't ask" preference.
    bool open_prefs = QGuiApplication::queryKeyboardModifiers() & Qt::ShiftModifier;

    bool show = true;
    if (std::strlen(imod->get_id()) > 21) {
        Inkscape::Preferences *prefs = Inkscape::Preferences::get();
        bool ask = prefs->getBool("/dialogs/import/ask") || open_prefs;
        bool ask_svg = prefs->getBool("/dialogs/import/ask_svg") || open_prefs;
        ...
        } else if (Glib::ustring(imod->get_id(), 21).compare("org.inkscape.input.qt") == 0) {
            if (!ask) {
                show = false;
                imod->set_gui(false);
            }
