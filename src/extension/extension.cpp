...
class AutoGUI : public QWidget {
public:
    /** \brief  Create an AutoGUI object */
    AutoGUI () : QWidget(nullptr) {
        _layout = new QVBoxLayout(this);
        _layout->setContentsMargins(InxParameter::GUI_BOX_MARGIN, InxParameter::GUI_BOX_MARGIN,
                                    InxParameter::GUI_BOX_MARGIN, InxParameter::GUI_BOX_MARGIN);
        _layout->setSpacing(InxParameter::GUI_BOX_SPACING);
    };
     ...
     */
    void addWidget(QWidget* widg, char const* tooltip, int indent) {
        if (widg) {
            widg->setContentsMargins(indent * InxParameter::GUI_INDENTATION, 0, 0, 0);
            _layout->addWidget(widg);

            if (tooltip) {
                widg->setToolTip(QString::fromUtf8(tooltip));
            }
        }
    };
...
*/
QWidget* Extension::autogui(SPDocument* doc, Inkscape::XML::Node* node, sigc::signal<void()>* changeSignal) {
    if (!_gui || widget_visible_count() == 0) {
        return nullptr;
    }

    const auto agui = new AutoGUI();
    ...
    for (const auto& widget : _widgets) {
        ...
        QWidget* widg = widget->get_widget(changeSignal);
        const char* tip = widget->get_tooltip();
...
QWidget* Extension::get_params_widget() {
    auto const retval = new QWidget();
    auto const layout = new QVBoxLayout(retval);
    auto const content = new QLabel(QStringLiteral("Params"));
