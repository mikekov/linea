                               ...
                               std::list<std::string> &params, bool ignore_stderr, bool pipe_diffs)
{
    ...
    if (new_xmldoc) {
        ...
        Inkscape::UI::gui_warning(_("The output from the extension could not be parsed."), 0 /*QT TODO parent_window*/);
    }
}
                             ...
                             const Glib::ustring& message)
{
    QMessageBox box(icon,
                    QString::fromUtf8(_("Inkscape")),
                    QString::fromStdString(message.raw()),
                    QMessageBox::Ok,
                    parent_window);
    box.setInformativeText(QString::fromStdString(data.raw()));

    // Add a scrollable text view for long stderr output
    auto const text_edit = new QTextEdit();
    text_edit->setReadOnly(true);
    text_edit->setLineWrapMode(QTextEdit::WidgetWidth);
    text_edit->setPlainText(QString::fromStdString(data.raw()));
    ...
    auto const layout = qobject_cast<QVBoxLayout*>(box.layout());
    if (layout) {
        layout->insertWidget(layout->count() - 1, scroll, 1);
    }
                    ...
                    Glib::ustring const &filein, file_listener &fileout, bool ignore_stderr, bool pipe_diffs)
{
    ...
    if (!stderr_data.empty() && !ignore_stderr) {
        if (INKSCAPE.use_gui()) {
            showPopupError(stderr_data, QMessageBox::Information,
