...
bool PageManager::isPointInViewport(Geom::Point pos) const {
    if (hasPages()) {
        return false;
    }
    ...
    bounds->expandBy(-0.1); // see sp-page getSensitiveRect
...
bool PageManager::subset(SPAttr key, const gchar *value)
{
    switch (key) {
        ...
        case SPAttr::INKSCAPE_DESK_CHECKERBOARD:
            // not used; use page color opacity < 1.0
