...
// i18n shim until Qt tr() is wired up for this translation unit
#ifndef _
#define _(s) (s)
#endif
...
void PatternManager::init() {
    ...
    for (auto doc : stock) {
        ...
        std::string name = doc->getDocumentName();
    ...
    for (auto pat : all) {
        ...
        _cache[pat] = std::shared_ptr<PatternItem>();
    }
...
// still needed by renderer; to be deleted
static Cairo::RefPtr<Cairo::Surface> create_pattern_surface(SPDocument &sandbox,
...
static QImage create_pattern_image(SPDocument &sandbox,
    char const* name, SPDocument &source, double scale,
    std::optional<uint32_t> checkerboard = {})
{
    // Retrieve the pattern named 'name' from the source SVG document
...
static std::shared_ptr<PatternItem> create_hatch_item(SPHatch* hatch, bool stock_hatch, double scale) {
    if (!hatch) return {};
...
// input pattern can be a link or a root pattern
static std::shared_ptr<PatternItem> create_pattern_item(SPPattern* pattern, bool stock_pattern, double scale) {
    if (!pattern) return {};
...
Cairo::RefPtr<Cairo::Surface> PatternManager::get_image_surface(SPPaintServer* pattern, int width, int
height, double device_scale) {
    ...
    return create_pattern_surface(*_preview_doc, pattern->getId(), *pattern->document, device_scale);
}

QImage PatternManager::get_image(SPPaintServer* pattern, int width, int height, double device_scale) {
    if (!pattern) return {};
...
QImage PatternManager::get_preview(SPPaintServer* pattern, int width, int height, unsigned int rgba_background, double device_scale) {
    if (!pattern) return {};
...
std::shared_ptr<PatternItem> PatternManager::get_item(SPPaintServer* paint) {
    std::shared_ptr<PatternItem> item;
...
const std::vector<std::shared_ptr<PatternManager::Category>>& PatternManager::get_categories() {
    init();
    return _categories;
}
