...
public:
    ...
    // Global CSS custom properties from <style> elements. Shared across the document.
    void updateCssVariables();
    Glib::ustring resolveCssVariableValue(char const *value) const;
...
private:
    ...
    std::unique_ptr<QTimer> modified_connection;
    bool rerouting_connection_scheduled = false;
    // QObject used as a context/guard for QTimer::singleShot callbacks so they
    // are automatically cancelled when the document is destroyed.
    QObject _qtGuard;
    ...
    // CSS variables from global <style> elements
