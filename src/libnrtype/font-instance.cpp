...
void FontInstance::find_font_metrics()
{
    ...
    if (face->units_per_EM != 0) {  // If zero then it's a bitmap font.
        ...
        _underline_position  = std::fabs((double)face->underline_position  / face->units_per_EM);
        _underline_thickness = std::fabs((double)face->underline_thickness / face->units_per_EM);
...
FontGlyph const *FontInstance::LoadGlyph(unsigned int glyph_id)
{
    ...
    // Use actual underline metrics from the font instead of a rough 1.1x heuristic,
    // since underline_position can be deeper than descent_max (especially for fonts
    // with shallow descenders but deep underline metrics). Also account for overline
    // which sits at ascender - underline_position + thickness, and wavy amplitude
    // (vextent/20 ≈ 0.05 em). Add a small margin for safety.
    double deco_margin = 0.1; // extra margin for wavy amplitude, clamped thickness, etc.
    double deco_bottom = std::max(_ascent_max * 1.1,
                                  _ascent_max + _underline_position + _underline_thickness + deco_margin);
    double deco_top    = std::min(-_descent_max * 1.1,
                                  -(_underline_position + _underline_thickness + deco_margin));
    n_g->bbox_draw.setRight(n_g->h_advance);       // Advance
    n_g->bbox_draw.setBottom(deco_bottom);         // Expand to allow for text decoration (overline)
    n_g->bbox_draw.setTop(deco_top);               // Expand to allow for text decoration (underline)
