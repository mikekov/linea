...
RectKnotHolderEntityRY::knot_set(Geom::Point const &p, Geom::Point const &/*origin*/, unsigned int state)
{
    ...
    if (state & INK_CONTROL_MASK) { // When holding control then rx will be kept equal to ry,
...
Box3DKnotHolderEntity::knot_set_generic(SPItem *item, unsigned int knot_id, Geom::Point const &new_pos, unsigned int state)
{
    ...
    Box3D::Axis movement;
    if ((knot_id < 4) != (state & INK_SHIFT_MASK)) {
        movement = Box3D::XY;
    } else {
        movement = Box3D::Z;
    }

    box->set_corner (knot_id, s * i2dt, movement, (state & INK_CONTROL_MASK));
    box->set_z_orders();
    box->position_set();
}
...
Box3DKnotHolderEntityCenter::knot_set(Geom::Point const &new_pos, Geom::Point const &origin, unsigned int state)
{
    ...
    box->set_center(s * i2dt, origin * i2dt, !(state & INK_SHIFT_MASK) ? Box3D::XY : Box3D::Z,
                    state & INK_CONTROL_MASK);
...
ArcKnotHolderEntityStart::knot_set(Geom::Point const &p, Geom::Point const &/*origin*/, unsigned int state)
{
    ...
    if ((state & INK_CONTROL_MASK) && snaps) {
...
void SpiralKnotHolderEntityOuter::knot_set(Geom::Point const &p, Geom::Point const & /*origin*/, unsigned int state)
{
    ...
    if (state & INK_SHIFT_MASK) { // rotate without roll/unroll
        ...
        if (!(state & INK_ALT_MASK) && rad_new > 1e-3 && rad_new/spiral->rad < 2) {
