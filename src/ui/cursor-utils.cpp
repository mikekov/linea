...
CursorRenderResult render_svg_cursor(double scale, CursorInputParams const &in)
{
    ...
    // construct a key
    Key cursor_key = {"", in.theme_name, in.file_name,
    ...
    if (!root) {
        ...
        // for (auto const &theme_name : theme_names) {
            // for (auto const &theme_path : theme_paths) {
                full_file_path = Glib::build_filename(icons_path, in.theme_name, "cursors", in.file_name);
...
void set_svg_cursor(Inkscape::UI::Widget::Canvas& widget,
                    ...
                    std::optional<Colors::Color> fill,
                    std::optional<Colors::Color> stroke) {
    auto cursor = load_svg_cursor_qt(file_name, widget.devicePixelRatio());
...
QCursor load_svg_cursor_qt(const std::string& file_name, double device_pixel_ratio,
    ...
    auto params = CursorInputParams{
        ...
        // .icon_theme = Gtk::IconTheme::get_for_display(widget.get_display()),
        .file_name = file_name,
        .fill = fill.value_or(Colors::Color(0xffffffff)),
        .stroke = stroke.value_or(Colors::Color(0x000000ff))
    };
    auto res = render_svg_cursor(device_pixel_ratio, params);

    if (!res.surface) {
        g_warning("Failed to render SVG cursor: %s", file_name.c_str());
    ...
    // Adjust hotspot for device pixel ratio
    int hotspot_x = res.hotspot.x() / device_pixel_ratio;
    int hotspot_y = res.hotspot.y() / device_pixel_ratio;
// printf("cursor: %d, %d\n", hotspot_x, hotspot_y);
    // Create cursor with high-DPI support
