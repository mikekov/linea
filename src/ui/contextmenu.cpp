...
void AppendItemFromAction(QMenu* menu,
                                 std::string_view action_id,
                                 const QString& label, std::string_view dummy_icon = {}) {
    auto& registry = ActionRegistry::get();
    auto id = normalizeActionId(action_id);
    if (!registry.hasAction(id)) {
        printf("CtxMenu: action not available: %s\n", id.data());
        return;
    }
    auto item = actionToCustomMenuItem(id, label);
    item.icon = {};
    appendCustomAction(menu, item);
}
 ...
 */
static void create_clipboard_actions(QMenu* menu, bool const paste_only = false)
{
...
ContextMenu::ContextMenu(SPDesktop *desktop, SPObject *object, std::vector<SPItem*> const &items, bool hide_layers_and_objects_menu_item)
{
    ...
    auto gmenu         = this; // Main menu
    auto gmenu_section = gmenu; // Gio::Menu::create(); // Section (used multiple times)
    ...
    if (auto page = cast<SPPage>(object)) {
        ...
        if (!item) {
            ...
            // gmenu->append_section(
            create_clipboard_actions(gmenu, true);
            ...
            // gmenu->append_section(
            create_clipboard_actions(gmenu);
            ...
            // Dialogs
            auto gmenu_dialogs = gmenu; // Gio::Menu::create();
#if 0
            ...
            if (!is<SPAnchor>(item)) {
                ...
                // Selection
                // gmenu_section = Gio::Menu::create();
                auto gmenu_submenu = appendCustomSubmenu(gmenu, _("Select Same"));
                ...
                // gmenu->append_section(gmenu_section);
                appendCustomSeparator(gmenu);

                // Groups and Layers
                gmenu_section = gmenu; // Gio::Menu::create();
                ...
                if (is<SPGroup>(item)) {
                    ...
                    auto groupLabel = QString::fromStdString(item->defaultLabel());
                    if (groupLabel.size() > 30) {
                        groupLabel = groupLabel.left(27) + QStringLiteral("\u2026");
                    }
                    QString label = QString::fromStdString(Glib::ustring::compose(_("Enter Group %1"), groupLabel.toStdString()).raw());
        ...
        if (has_hidden_below_cursor || has_locked_below_cursor) {
        // gmenu_section = Gio::Menu::create();
        if (has_hidden_below_cursor) {
            appendCustomAction(gmenu_section, {
                .title = _("Unhide Objects Below Cursor"),
                .onTrigger = [this, document] { unhide_or_unlock(document, true); },
            });
        }
        if (has_locked_below_cursor) {
            appendCustomAction(gmenu_section, {
                .title = _("Unlock Objects Below Cursor"),
                .onTrigger = [this, document] { unhide_or_unlock(document, false); },
            });
        }
        // gmenu->append_section(gmenu_section);
        }
    ...
    if (auto const window = desktop->getLineaWindow()) {
