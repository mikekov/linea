...
/// Sentinel mimetype used internally to represent image clipboard contents.
...
private:
    ...
    // clipboard callbacks
    QByteArray _onGet(char const *mime_type);
    ...
    QClipboard* _clipboard = QGuiApplication::clipboard(); ///< Handle to the system wide clipboard - for convenience

    // For throttling rogue clipboard managers.
    std::optional<std::chrono::steady_clock::time_point> last_req;

    /// True when we put the current data on the clipboard. QClipboard::ownsClipboard()
    /// is unreliable on macOS (always returns false), so we track ownership ourselves.
    bool _we_own_clipboard = false;

    friend class InkscapeClipboardData;
};

/**
 * QMimeData subclass that lazily serializes the internal clipboard document on demand.
 * Qt calls retrieveData() when an external application requests a specific MIME type.
 */
class InkscapeClipboardData : public QMimeData
{
public:
    explicit InkscapeClipboardData(ClipboardManagerImpl *owner, QStringList formats)
        : _owner(owner)
        , _formats(std::move(formats))
    {}

    QStringList formats() const override { return _formats; }

protected:
    QVariant retrieveData(const QString &mimeType, QMetaType /*type*/) const override
    {
        QByteArray data = _owner->_onGet(mimeType.toUtf8().constData());
        if (data.isEmpty()) {
            return {};
        }
        return data;
      ...
      _text_style(nullptr)
{
    ...
    QObject::connect(QCoreApplication::instance(), &QCoreApplication::aboutToQuit,
                     [this] { _discardInternalClipboard(); });

    // Track clipboard ownership. QClipboard::ownsClipboard() is unreliable on
    // macOS (always returns false), so we use a flag + dataChanged signal.
    // When another app takes the clipboard, clear our internal copy.
    QObject::connect(_clipboard, &QClipboard::dataChanged, [this] {
        if (!_we_own_clipboard) {
            _discardInternalClipboard();
        }
        _we_own_clipboard = false;
    });
}
...
void ClipboardManagerImpl::copy(ObjectSet *set)
{
    if (auto const desktop = set->desktop()) {
        ...
        if (auto const text_tool = dynamic_cast<Tools::TextTool*>(desktop->getTool())) {
            _discardInternalClipboard();
            _clipboard->setText(QString::fromStdString(get_selected_text(*text_tool).raw()));
            if (_text_style) {
                sp_repr_css_attr_unref(_text_style);
                _text_style = nullptr;
            }
...
bool ClipboardManagerImpl::copyString(Glib::ustring str) {
    if (!str.empty()) {
        _discardInternalClipboard();
        _clipboard->setText(QString::fromStdString(str.raw()));
        return true;
    }
    return false;
}
...
bool ClipboardManagerImpl::_pasteImage(SPDocument *doc)
{
    ...
    // retrieve image data
    QImage img = _clipboard->image();
    if (img.isNull()) {
        return false;
    }

    auto const filename = get_tmp_filename("inkscape-clipboard-import");
    img.save(QString::fromStdString(filename), "PNG");
...
bool ClipboardManagerImpl::_pasteText(SPDesktop *desktop)
{
    ...
    if (!_clipboard->mimeData() || !_clipboard->mimeData()->hasText()) {
        return false;
    }
    Glib::ustring clip_text = _clipboard->text().toStdString();
...
{
    // Fast path: we own the clipboard, so the SPDoc is already populated.
    if (_we_own_clipboard && _clipboardSPDoc) {
        return;
    }
    ...
    if (best_target == "CF_ENHMETAFILE" || best_target == "WCF_ENHMETAFILE") {
        // Try to save clipboard data as an emf file (using win32 API).
    ...
    if (!file_saved) {
        const QMimeData *mime = _clipboard->mimeData();
        if (!mime) {
            return;
        }
        QByteArray data = mime->data(QString::fromStdString(best_target.raw()));
        if (data.isEmpty()) {
            return;
        }
        std::ofstream out(filename, std::ios::binary);
        if (!out) {
            std::cout << "Pasting failed: could not open temp file " << filename << std::endl;
            return;
        }
 ...
 */
QByteArray ClipboardManagerImpl::_onGet(char const *mime_type)
{
    if (!_clipboardSPDoc) {
        return {};
    }
    ...
    // Save to a temporary file, read it back and then return the contents.
    ...
    try {
        ...
        if (out == outlist.end()) {
            // No output extension handles this mimetype.
...
Glib::ustring ClipboardManagerImpl::_getBestTarget(SPDesktop *desktop)
{
    ...
    for (auto tgt : preferred_targets) {
        if (mime->hasFormat(QString::fromUtf8(tgt))) {
            return tgt;
        }
    }
#ifdef _WIN32
 ...
 *
 * Creates an InkscapeClipboardData instance that will lazily serialize the
 * internal clipboard document via _onGet() when an external app requests data.
 */
        ...
        if (!out->deactivated()) {
            ...
            if (mime != CLIPBOARD_TEXT_TARGET) {
                if (!plaintextSet && mime.find("svg") == Glib::ustring::npos) {
                    formats << QString::fromUtf8(CLIPBOARD_TEXT_TARGET);
                    plaintextSet = true;
                }
                formats << QString::fromStdString(mime.raw());
            }
        }
    }

    // Add PNG export explicitly since there is no extension for this.
    // On Windows, Qt will also present this as CF_DIB/CF_BITMAP.
    formats << "image/png";

    // Block dataChanged so our own setMimeData doesn't clear the ownership flag.
    {
        QSignalBlocker blocker(_clipboard);
        _we_own_clipboard = true;
        _clipboard->setMimeData(new InkscapeClipboardData(this, std::move(formats)));
    }
    last_req = std::chrono::steady_clock::now();
}
...
{
    _clipboard->setText(QString::fromStdString(color.toString()));
}
