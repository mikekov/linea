...
void MeasureTool::knotClickHandler(SPKnot *knot, guint state)
{
    if (state & INK_SHIFT_MASK) {
        ...
        // QT TODO: Dialog::KnotPropertiesDialog::showDialog(_desktop, knot, unit_name);
    }
}
...
bool MeasureTool::root_handler(CanvasEvent const &event)
{
    ...
    [&] (KeyPressEvent const &event) {
        if (event.keyval == INK_KEY_Control_L || event.keyval == INK_KEY_Control_R) {
            explicit_base_tmp = explicit_base;
            explicit_base = end_p;
            showInfoBox(last_pos, true);
        }
        if ((event.modifiers & INK_ALT_MASK ) && ((event.keyval == INK_KEY_c) || (event.keyval == INK_KEY_C))) {
            copyToClipboard();
            ret = true;
        }
    },
    ...
    [&] (MotionEvent const &event) {
        if (!(event.modifiers & INK_BUTTON1_MASK)) {
            ...
            last_pos = event.pos;
            showInfoBox(last_pos, event.modifiers & INK_CONTROL_MASK);
        } else {
            if (pathmeasure) {
    ...
    [&] (ButtonReleaseEvent const &event) {
        ...
        bool shift = event.modifiers & INK_SHIFT_MASK;
