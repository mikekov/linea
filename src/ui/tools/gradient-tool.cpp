...
bool GradientTool::root_handler(CanvasEvent const &event)
{
    ...
    [&] (ButtonPressEvent const &event) {
        ...
        if (event.num_press == 2) {
            ...
            if (event.modifiers & INK_SHIFT_MASK && !(event.modifiers & INK_CONTROL_MASK)) {
                ...
                if (!(event.modifiers & INK_CONTROL_MASK)) {
                    item_to_select = sp_event_context_find_item(_desktop, event.pos, event.modifiers & INK_ALT_MASK, true);
                }
