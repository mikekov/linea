...
bool Box3dTool::root_handler(CanvasEvent const &event)
{
    ...
    inspect_event(event,
        [&] (ButtonPressEvent const &event) {
            ...
            item_to_select = sp_event_context_find_item(_desktop, button_w, event.modifiers & INK_ALT_MASK, event.modifiers & INK_CONTROL_MASK);
    ...
    [&] (MotionEvent const &event) {
        if (dragging && event.modifiers & INK_BUTTON1_MASK) {
            ...
            ctrl_dragged  = event.modifiers & INK_CONTROL_MASK;

            if (event.modifiers & INK_SHIFT_MASK && box3d) {
