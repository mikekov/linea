...
{
    blink_time = QGuiApplication::styleHints()->cursorFlashTime();
    if (blink_time <= 0) {
        blink_time = 200;
    }
    ...
    /* QT-specific start */
    // Qt handles IM focus automatically; just connect the commit signal.
    // Preedit is intentionally ignored (see bug 1086769 — multi-byte preedit
    // was broken in Inkscape; we only care about the final committed string).
    im_commit_conn = _desktop->getCanvas()->connectIMCommit(
        sigc::mem_fun(*this, &TextTool::_onIMCommit));
    /* QT-specific end */
...
bool TextTool::root_handler(CanvasEvent const &event)
{
    ...
    inspect_event(event,
        ...
        [&] (MotionEvent const &event) {
            if (creating && event.modifiers & INK_BUTTON1_MASK) {
                ...
                gobble_motion_events(INK_BUTTON1_MASK);
            ...
            if (event.modifiers & INK_BUTTON1_MASK && dragging_state) {
        ...
        [&] (ButtonReleaseEvent const &event) {
            ...
            if (creating && within_tolerance) {
...

                {
                    ...
                    _desktop->getCanvas()->setIMCursorRect(QRect(
                        std::floor(im_rect.left()),  std::floor(im_rect.top()),
                        std::floor(im_rect.width()), std::floor(im_rect.height())));
                }
        ...
        [&] (KeyPressEvent const &event) {
            ...
            if (group0_keyval == INK_KEY_KP_Add || group0_keyval == INK_KEY_KP_Subtract) {
                if (!(event.modifiers & INK_CONTROL_MASK)) { // mod2 is NumLock; if on, type +/- keys
            ...
            if (text || nascent_object) {
                ...
                // In Qt, keys consumed by the IM never reach keyPressEvent(),
                // so we always handle keys that arrive here — there is no
                // equivalent of gtk_im_context_filter_keypress() needed.
                {
                    ...
                    if (!mod_ctrl_only(event) && unimode) {
                        ...
                        switch (group0_keyval) {
                            ...
                            default: {
                                auto const xdigit = event.text[0]; // gdk_keyval_to_unicode(group0_keyval);
                    ...
                    switch (group0_keyval) {
                        ...
                        case INK_KEY_KP_End:
                            ...
                            ret = true;
                            return;
                        case INK_KEY_Page_Down:
                        case INK_KEY_KP_Page_Down:
                            if (text) {
                            ...
                            ret = true;
                            return;
                        case INK_KEY_Page_Up:
                        case INK_KEY_KP_Page_Up:
                            if (text) {
                    ...
                    /* QT-specific start */
                    // In Qt, plain printable key presses are not delivered via
                ...
                if ((group0_keyval == INK_KEY_Up    ||
                     group0_keyval == INK_KEY_Down  ||
                     group0_keyval == INK_KEY_KP_Up ||
                     group0_keyval == INK_KEY_KP_Down )
                    && !mod_ctrl_only(event))
                {
                    ret = true;
                } else if (group0_keyval == INK_KEY_Escape) { // cancel rubberband
                    if (creating) {
                        creating = false;
                        ungrabCanvasEvents();
                        Rubberband::get(_desktop)->stop();
                    }
                } else if ((group0_keyval == INK_KEY_x || group0_keyval == INK_KEY_X) && mod_alt_only(event)) {
        ...
        [&] (KeyReleaseEvent const &event) {
            // Qt: IM key filtering is automatic — nothing to do here.
            (void)event;
        },
        [&] (CanvasEvent const &event) {}
    );
...
std::optional<Linea::Props::EditTarget> TextTool::targetSelection() {
    if (!text) return std::nullopt;

    _validateCursorIterators();
    if (text_sel_start == text_sel_end) return std::nullopt;

    return Linea::Props::EditTarget{text, text_sel_start, text_sel_end};
}
...
void TextTool::_updateCursor(bool scroll_to_see)
{
    if (text) {
        ...
        /* fixme: ... need another transformation to get canvas widget coordinate space? */
        {
            ...
            _desktop->getCanvas()->setIMCursorRect(QRect(
                std::floor(im_rect.left()),  std::floor(im_rect.top()),
                std::max(1, (int)std::floor(im_rect.width())),
                std::max(1, (int)std::floor(im_rect.height()))));
        }
...
void TextTool::_onIMCommit(const QString& str) {
    if (!text) {
    ...
    // Insert the new text
    Glib::ustring const ustr = str.toStdString();
    text_sel_start = text_sel_end = sp_te_replace(text, text_sel_start, text_sel_end, ustr.c_str());
