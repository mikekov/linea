...
public:
    ...
    virtual bool usesInputMethod() const { return false; }

    /* QT-specific start */
    /// Returns true when the tool is actively consuming keyboard input (e.g. text
    /// tool with a cursor placed), so the canvas can suppress window-level shortcut
    /// actions via QEvent::ShortcutOverride.
    virtual bool wantsAllKeys() const { return false; }
    /* QT-specific end */
...
public:
    ...
    void use_cursor(std::optional<QCursor> cursor);
    QCursor get_cursor(QWidget& widget, std::string const &filename) const;
...
unsigned get_latin_keyval_impl(unsigned event_keyval, unsigned event_keycode,
                               unsigned event_state, unsigned event_group,
