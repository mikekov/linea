...
public:
    ...
    // Returns a text-range EditTarget for the current cursor subselection,
    // or nullopt if there is no active text or the selection is empty.
    // Validates the cursor iterators before returning.
    std::optional<Linea::Props::EditTarget> targetSelection();
...
protected:
    ...
    /* QT-specific start */
    /// True whenever the text cursor is placed (active text object or nascent object),
    /// so QEvent::ShortcutOverride suppresses window-level shortcuts.
    bool wantsAllKeys() const override { return text != nullptr || nascent_object; }

    /// True whenever the tool uses an input method (IME) for text input.
    bool usesInputMethod() const override { return true; }
    /* QT-specific end */
...
private:
    ...
    sigc::scoped_connection im_commit_conn;
    ...
    void _onIMCommit(const QString& str);
