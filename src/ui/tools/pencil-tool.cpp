...
void PencilTool::_endpointSnap(Geom::Point &p, guint const state) {
    if ((state & INK_CONTROL_MASK)) { //CTRL enables constrained snapping
        if (this->_npoints > 0) {
...
bool PencilTool::_handleMotionNotify(MotionEvent const &event) {
    if ((event.modifiers & INK_CONTROL_MASK) && (event.modifiers & INK_BUTTON1_MASK)) {
    ...
    if ((event.modifiers & INK_BUTTON2_MASK)) {
        // allow scrolling
        return false;
    }
    ...
    if ((event.modifiers & INK_BUTTON1_MASK) && _is_drawing) {
        /* Grab mouse, so release will not pass unnoticed */
        grabCanvasEvents();
    }
    ...
    if (event.button == 1 && _is_drawing) {
        ...
        switch (_state) {
            case SP_PENCIL_CONTEXT_IDLE:
                ...
                if (!(event.modifiers & INK_CONTROL_MASK) && !is_tablet) {
            ...
            case SP_PENCIL_CONTEXT_FREEHAND:
                if (event.modifiers & INK_ALT_MASK && !tablet_enabled) {
