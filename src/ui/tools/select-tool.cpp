...
key_is_a_modifier (guint key) {
    return (key == INK_KEY_Alt_L     ||
            key == INK_KEY_Alt_R     ||
            key == INK_KEY_Control_L ||
            key == INK_KEY_Control_R ||
            key == INK_KEY_Shift_L   ||
            key == INK_KEY_Shift_R   ||
            key == INK_KEY_Meta_L    ||  // Meta is when you press Shift+Alt (at least on my machine)
            key == INK_KEY_Meta_R);
}
...
bool SelectTool::root_handler(CanvasEvent const &event)
{
    ...
    inspect_event(event,
        ...
        [&] (MotionEvent const &event) {
            ...
            if (grabbed && event.modifiers & (INK_SHIFT_MASK | INK_ALT_MASK)) {
                    ...
                    if (r->isStarted() && !within_tolerance) {
                        ...
                        if (add_to && !rb_escaped && !drag_escaped) {
                            ...
                            if (!(rb_escaped) && !(drag_escaped) && !force_drag) {
                                ...
                                auto point = _desktop->w2d(event.pos);
                                auto& page_manager = document->getPageManager();
                                auto page = page_manager.findPageAt(point);

                                if (page) {
                                    selection->set(page);
                                }
                                else if (page_manager.isPointInViewport(point)) {
                                    selection->set(document->getRoot());
                                }
                                else {
                                    if (!selection->isEmpty()) {
                                        selection->clear();
        ...
        [&] (KeyPressEvent const &event) {
            ...
            bool alt = keyval == INK_KEY_Alt_L  ||
                       keyval == INK_KEY_Alt_R  ||
                       keyval == INK_KEY_Meta_L ||
            ...
            switch (keyval) {
                case INK_KEY_Left: // move selection left
                ...
                case INK_KEY_Up: // move selection up
                case INK_KEY_KP_Up:
                    if (!mod_ctrl(event)) { // not ctrl
                ...
                case INK_KEY_Right: // move selection right
                ...
                case INK_KEY_Down: // move selection down
                ...
                case INK_KEY_C:
                    ...
                    if (dragging && grabbed) {
                        _seltrans->stamp(keyval != INK_KEY_space);
                        ret = true;
                    }
                    break;
