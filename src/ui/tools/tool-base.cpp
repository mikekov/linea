    ...
    , _origin(origin)
{
    ...
    _event->time = 0;//TODO GDK_CURRENT_TIME;
}
 ...
 */
QCursor ToolBase::get_cursor(QWidget& widget, std::string const &filename) const {
    ...
    return load_svg_cursor_qt(filename, widget.devicePixelRatio(), fillColor, strokeColor);
}
...
{
    if (auto widget = _desktop->getCanvas()) {
        widget->setCursor(get_cursor(*widget, _cursor_filename));
    }
 ...
 *
 * If the optional is empty, sets the remembered cursor (reverting it)
 */
void ToolBase::use_cursor(std::optional<QCursor> cursor) {
    if (cursor) {
        _desktop->getCanvas()->setCursor(*cursor);
    } else {
        use_tool_cursor();
    }
}
...
static void sp_toggle_selector(SPDesktop *dt) {
    ...
    if (dynamic_cast<Inkscape::UI::Tools::SelectTool *>(dt->getTool())) {
        ...
        switch_selector_to = dt->getActiveTool();
...
void sp_toggle_dropper(SPDesktop *dt)
{
    ...
    if (dynamic_cast<Inkscape::UI::Tools::DropperTool *>(dt->getTool())) {
        ...
        switch_dropper_to = dt->getActiveTool();
...
bool ToolBase::root_handler(CanvasEvent const &event)
{
    ...
    [&] (ButtonPressEvent const &event) {
        if (event.num_press == 2) {
            ...
            check_lost_button_up = !(event.device && event.device->source != InputSource::TABLET_PAD);
            ...
            case 2:
                if (event.modifiers & INK_CONTROL_MASK && !_desktop->get_rotation_lock()) {
                ...
                } else if (event.modifiers & INK_SHIFT_MASK) {
                    zoom_rb = 2;
                } else {
                    // When starting panning, make sure there are no snap events pending because these might disable the panning again
                    if (_uses_snap) {
            ...
            case 3:
                if (event.modifiers & (INK_SHIFT_MASK | INK_CONTROL_MASK)) {
                    // When starting panning, make sure there are no snap events pending because these might disable the panning again
                    if (_uses_snap) {
                        discard_delayed_snap_event();
                    }
    ...
    [&] (MotionEvent const &event) {
        if (panning) {
            ...
            if (check_lost_button_up && ((panning == 2 && !(event.modifiers & INK_BUTTON2_MASK)) ||
                                         (panning == 1 && !(event.modifiers & INK_BUTTON1_MASK)) ||
                                         (panning == 3 && !(event.modifiers & INK_BUTTON3_MASK)))) {
                ...
                if (panning_cursor == 0) {
                    panning_cursor = 1;
                    auto window = _desktop->getCanvas();
            ...
            if (zoom_rb == 2) {
                gobble_motion_events(INK_BUTTON2_MASK);
            }
        } else if (rotating) {
    ...
    [&] (ButtonReleaseEvent const &event) {
        ...
        if (panning_cursor == 1) {
            panning_cursor = 0;
            _desktop->getCanvas()->setCursor(_cursor);
        }
        ...
        if (middle_mouse_zoom && within_tolerance && (panning || zoom_rb)) {
            ...
            _desktop->zoom_relative(event_dt, (event.modifiers & INK_SHIFT_MASK) ? 1 / zoom_inc : zoom_inc);
            ret = true;
    ...
    [&] (ScrollEvent const &event) {
        ...
        if (action == Type::CANVAS_ROTATE) {
            ...
            double angle;
            if (event.unit == Inkscape::ScrollUnit::WHEEL) {
...
void ToolBase::set_on_buttons(CanvasEvent const &event)
{
    inspect_event(event,
        ...
        [&] (MotionEvent const &event) {
            _button1on = event.modifiers & INK_BUTTON1_MASK;
            _button2on = event.modifiers & INK_BUTTON2_MASK;
            _button3on = event.modifiers & INK_BUTTON3_MASK;
        },
        [&] (CanvasEvent const &event) {}
    );
}
    ...
    inspect_event(event,
        [&] (ButtonPressEvent const &event) {
            if (!are_buttons_1_and_3_on(event) && event.button == 3 &&
                !(event.modifiers & (INK_SHIFT_MASK | INK_CONTROL_MASK))) {
                menu_popup(event);
                ret = true;
...
void ToolBase::setup_for_drag_start(ButtonPressEvent const &ev)
{
    ...
    item_to_select = sp_event_context_find_item(_desktop, ev.pos, ev.modifiers & INK_ALT_MASK, true);
}
...
void ToolBase::menu_popup(CanvasEvent const &event, SPObject *obj)
{
    ...
    auto const popup = [&] (std::optional<Geom::Point> const &pos) {
        ...
        auto menu = new Linea::UI::ContextMenu(_desktop, obj, items_under_cursor);
        menu->setAttribute(Qt::WA_DeleteOnClose);

        // Map the canvas-relative position to global screen coordinates for popup.
        auto canvas = _desktop->getCanvas();
        QPoint globalPos;
        if (pos) {
            globalPos = canvas->mapToGlobal(QPoint(static_cast<int>(pos->x()), static_cast<int>(pos->y())));
        } else {
                                ...
                                char const *alt_tip)
{
    ...
    bool ctrl =  ctrl_tip  && (mod_ctrl(event)  || keyval == INK_KEY_Control_L || keyval == INK_KEY_Control_R);
    bool shift = shift_tip && (mod_shift(event) || keyval == INK_KEY_Shift_L   || keyval == INK_KEY_Shift_R);
    bool alt =   alt_tip   && (mod_alt(event)   || keyval == INK_KEY_Alt_L     || keyval == INK_KEY_Alt_R
                                                || keyval == INK_KEY_Meta_L    || keyval == INK_KEY_Meta_R);
...
{
    // Qt: no GDK keyboard/seat API to monitor layout changes.
}

unsigned get_latin_keyval_impl(unsigned const event_keyval, unsigned const event_keycode,
                               unsigned const event_state, unsigned const event_group,
                               unsigned *consumed_modifiers)
{
    // Qt: no GDK display — the keyval from the event converter is already
    // in GDK key value space, so return it directly.
    (void)event_keycode;
    (void)event_state;
    (void)event_group;
...
void ToolBase::snap_delay_handler(gpointer item, gpointer item2, MotionEvent const &event, DelayedSnapEvent::Origin origin)
{
    ...
    bool const c1 = event.modifiers & INK_BUTTON2_MASK; // We shouldn't hold back any events when other mouse buttons have been
    bool const c2 = event.modifiers & INK_BUTTON3_MASK; // pressed, e.g. when scrolling with the middle mouse button; if we do then
...
void ToolBase::process_delayed_snap_event()
{
    ...
    case DelayedSnapEvent::GUIDE_VRULER: {
        // QT TODO: CanvasGrid not compiled in Qt port
...
void ToolBase::set_name(const std::string& name) {
    _tool_name = name;
}

const std::string& ToolBase::get_name() const {
    return _tool_name;
}
