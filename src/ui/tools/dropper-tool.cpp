...
bool DropperTool::root_handler(CanvasEvent const &event)
{
    ...
    stroke   = modifiers & INK_SHIFT_MASK;
    dropping = modifiers & INK_CONTROL_MASK; // Even on macOS.
    invert   = modifiers & INK_ALT_MASK;
    ...
    inspect_event(event,
        ...
        [&] (MotionEvent const &event) {
            if (event.modifiers & (INK_BUTTON2_MASK | INK_BUTTON3_MASK)) {
