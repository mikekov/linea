...
bool MeshTool::root_handler(CanvasEvent const &event)
{
    ...
    inspect_event(event,
            ...
            if (event.num_press == 1 && event.button == 1) {
                ...
                if (!over_curve.empty() && contains_mesh) {
                    for (auto it : over_curve) {
                        ...
                        bool add    = (event.modifiers & INK_SHIFT_MASK);
                        bool toggle = (event.modifiers & INK_CONTROL_MASK);
                        if ( !add && !toggle ) {
                            _grdrag->deselectAll();
                        }
                    ...
                    // Check if object already has mesh... if it does,
                    // don't create new mesh with click-drag.
                    if (contains_mesh && !(event.modifiers & INK_CONTROL_MASK)) {
                    ...
                    if (!(event.modifiers & INK_CONTROL_MASK)) {
                        item_to_select = sp_event_context_find_item (_desktop, button_w, event.modifiers & INK_ALT_MASK, TRUE);
                    }
        ...
        [&] (MotionEvent const &event) {
            // Mouse move
            if (dragging && (event.modifiers & INK_BUTTON1_MASK)) {
        ...
        [&] (ButtonReleaseEvent const &event) {
            ...
            if (event.button == 1) {
                ...
                if ( (event.modifiers & INK_CONTROL_MASK) && (event.modifiers & INK_ALT_MASK ) ) {
                    ...
                    if (event.modifiers & INK_CONTROL_MASK && !(event.modifiers & INK_SHIFT_MASK)) {
        ...
        [&] (KeyPressEvent const &event) {
...
#ifdef DEBUG_MESH
            std::cout << "root_handler: INK_KEY_PRESS" << std::endl;
#endif
            ...
            switch (get_latin_keyval (event)) {
                ...
                case INK_KEY_b:  // Toggle mesh side between lineto and curveto.
                case INK_KEY_B:
                ...
                case INK_KEY_c:  // Convert mesh side from generic Bezier to Bezier approximating arc,
                case INK_KEY_C:  // preserving handle direction.
                    if (mod_alt(event) && _grdrag->isNonEmpty() && _grdrag->hasSelection()) {
                ...
                case INK_KEY_g:  // Toggle mesh tensor points on/off
                case INK_KEY_G:
                    if (mod_alt(event) && _grdrag->isNonEmpty() && _grdrag->hasSelection()) {
                ...
                case INK_KEY_j:  // Smooth corner color
                case INK_KEY_J:
                    if (mod_alt(event) && _grdrag->isNonEmpty() && _grdrag->hasSelection()) {
                ...
                case INK_KEY_k:  // Pick corner color
                case INK_KEY_K:
                    if (mod_alt(event) && _grdrag->isNonEmpty() && _grdrag->hasSelection()) {
