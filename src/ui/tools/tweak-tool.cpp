    ...
    inspect_event(event,
        ...
        [&] (MotionEvent const &event) {
            ...
            if (is_drawing && ( event.modifiers & INK_BUTTON1_MASK )) {
                sp_tweak_dilate (this, event.pos, motion_doc, motion_doc - last_push, event.modifiers & INK_SHIFT_MASK? true : false);
        ...
        [&] (ButtonReleaseEvent const &event) {
            ...
            if (is_dilating && event.button == 1) {
                if (has_dilated) {
                    ...
                    pressure = 0.03;
                    sp_tweak_dilate(this, event.pos, _desktop->dt2doc(motion_dt), Geom::Point(0, 0), event.modifiers & INK_SHIFT_MASK);
                }
                is_dilating = false;
                has_dilated = false;
