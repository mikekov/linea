...
bool SprayTool::root_handler(CanvasEvent const &event)
{
    ...
    inspect_event(event,
        ...
        [&] (MotionEvent const &event) {
            ...
            // Dilating:
            if (is_dilating && ( event.modifiers & INK_BUTTON1_MASK )) {
                sp_spray_dilate(this, motion_doc, motion_doc - last_push, event.modifiers & INK_SHIFT_MASK ? true : false);
        ...
        [&] (ButtonReleaseEvent const &event) {
            ...
            if ((single_click || this->is_dilating) && event.button == 1) {
                if (single_click) {
                    sp_spray_dilate(this, _desktop->dt2doc(motion_dt), motion_doc - this->last_push, event.modifiers & INK_SHIFT_MASK? true : false, true);
                } else if (!this->has_dilated) {
                    // If we did not rub, do a light tap
                    pressure = 0.03;
                    sp_spray_dilate(this, _desktop->dt2doc(motion_dt), Geom::Point(0,0), event.modifiers & INK_SHIFT_MASK);
                }
