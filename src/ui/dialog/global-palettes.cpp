...
// Load CSS @theme color palette
void load_css_palette(PaletteFileData& palette, const std::string& path) {
    palette.name = Glib::path_get_basename(path);
    auto ext = get_extension(palette.name);
    if (ext == ".css") {
        palette.name = palette.name.substr(0, palette.name.size() - ext.size());
    ...
    static auto const regex_var = Glib::Regex::create(
        "^\\s*(--[A-Za-z0-9_-]+)\\s*:\\s*(.+?)(?:\\s*;)?\\s*$",
        Glib::Regex::CompileFlags::OPTIMIZE | Glib::Regex::CompileFlags::ANCHORED);
    ...
    while (std::fgets(buf, sizeof(buf), f.get())) {
        ...
        if (line.find("@theme") != Glib::ustring::npos) {
            in_theme = true;
            continue;
        }
        if (!in_theme) continue;
        if (line.find('}') != Glib::ustring::npos) {
            in_theme = false;
            continue;
        }

        Glib::MatchInfo match;
        if (regex_var->match(line, match)) {
            auto var = match.fetch(1);
            auto value = match.fetch(2);
            ...
            if (auto color = Color::parse(value.raw())) {
                color->setName(name.raw());
                palette.colors.emplace_back(std::move(*color));
            } else {
                g_warning("Could not parse color value %s", value.c_str());
            }
        }
    }
}
...
{
    // Load the palettes.
    for (auto const &path : Inkscape::IO::Resource::get_filenames(Inkscape::IO::Resource::PALETTES, {".gpl", ".acb", ".ase", ".css"})) {
