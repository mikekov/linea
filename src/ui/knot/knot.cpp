...
void SPKnot::startDragging(Geom::Point const &p, Geom::IntPoint const &xy, uint32_t etime)
{
    ...
    if (!nograb && ctrl) {
        ctrl->grab(KNOT_EVENT_MASK, _cursors[SP_KNOT_STATE_DRAGGING].value_or(QCursor()));
    }
...
bool SPKnot::eventHandler(Inkscape::CanvasEvent const &event)
{
    ...
    [&] (Inkscape::MotionEvent const &event) {
        if (!(event.modifiers & INK_BUTTON1_MASK) && flags & SP_KNOT_DRAGGING) {
            pressure = 0;
...
void SPKnot::handler_request_position(Inkscape::MotionEvent const &event)
{
    ...
    if (event.modifiers & INK_BUTTON1_MASK) {
        Inkscape::UI::Tools::gobble_motion_events(INK_BUTTON1_MASK);
    }
}
...
void SPKnot::setCursor(SPKnotStateType type, QCursor cursor)
{
