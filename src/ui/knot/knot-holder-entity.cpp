...
{
    if (state & INK_SHIFT_MASK) { // Don't snap when shift-key is held
        return p;
    }
...
void PatternKnotHolderEntityScale::knot_set(Geom::Point const &p, Geom::Point const &, unsigned state)
{
    ...
    // If INK_CONTROL_MASK, then constrained snapping should be used.
    ...
    Affine new_transform = (state & INK_CONTROL_MASK) ? Scale((scale_x + scale_y) * 0.5)
...
{
    // FIXME: this snapping should be done together with knowing whether control was pressed. If INK_CONTROL_MASK, then constrained snapping should be used.
    Geom::Point p_snapped = snap_knot_position(p, state);
