...
void set_icon_sizes(Gtk::Widget *parent, int pixel_size)
{
    ...
    //     if (dynamic_cast<Gtk::SpinButton*>(&widget)) { // QT TODO: InkSpinButton not available
...
void gui_warning(const std::string &msg, QWidget *parent_window) {
    g_warning("%s", msg.c_str());
    if (!parent_window) {
        if (auto desktop = INKSCAPE.active_desktop()) {
            parent_window = desktop->getLineaWindow();
        }
    }
    QMessageBox::warning(parent_window, QString::fromUtf8(_("Warning")),
...
bool contains_focus(QWidget& widget)
{
    // if (widget.has_focus()) {
    //     return true;
    // }
...
void set_defocus_target(Gtk::Widget* panel, DefocusTarget* target) {
    ...
    for_each_descendant(*panel, [target](auto& widget) {
        // QT TODO: InkSpinButton not available
...
QString get_synthetic_object_name(const SPObject* object) {
    if (!object) return {};

    auto id = object->getId();
    if (auto item = cast<SPItem>(object)) {
        auto name = QString::fromUtf8(item->displayName());
        if (id) return name + " " + QString::fromUtf8(id);
        return name;
    }
    if (id) return QStringLiteral("#") + QString::fromUtf8(id);
    if (auto repr = object->getRepr()) return QStringLiteral("<") + QString::fromUtf8(repr->name()) + QStringLiteral(">");
...
void set_toggle_button_state(Gtk::ToggleButton& button, Linea::mixed_property<bool> prop, bool default_value) {
...
/*
void set_spin_button_value(Inkscape::UI::Widget::InkSpinButton& button, Linea::mixed_property<double> prop, std::optional<double> not_set_value) {
...
void set_spin_scale_value(Inkscape::UI::Widget::SpinScale& scale, Linea::mixed_property<double> prop, std::optional<double> not_set_value) {
...
QImage cairo_surface_to_qimage(cairo_surface_t* surface) {
    if (!surface) {
        return QImage();
    }

    // Ensure the surface is an image surface
    if (cairo_surface_get_type(surface) != CAIRO_SURFACE_TYPE_IMAGE) {
        g_warning("cairo_surface_to_qimage: surface is not an image surface");
        return QImage();
    }

    // Reference the surface to keep data alive
    ...
    // Convert Cairo format to QImage format
    ...
    switch (format) {
        ...
        case CAIRO_FORMAT_A8:
            qformat = QImage::Format_Alpha8;
            break;
        default:
            // Unsupported format, return empty image
            g_warning("cairo_surface_to_qimage: unsupported Cairo format %d", static_cast<int>(format));
            return QImage();
    }

    // create an image that will take ownership of the data and free it when done
    QImage image(data, width, height, stride, qformat,
        [](void* info) { cairo_surface_destroy(static_cast<cairo_surface_t*>(info)); },
        surface);

    image.setDevicePixelRatio(scale_x);

    return image;
}

void add_drop_shadow(QWidget* widget, int offset_x, int offset_y, int blur_radius, int alpha) {
