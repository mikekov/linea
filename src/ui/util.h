...
void gui_warning(const std::string &msg, QWidget *parent_window = nullptr);
...
// [[nodiscadk_keyval_to_unicoderd]] bool is_descendant_of(Gtk::Widget const &descendant, Gtk::Widget const &ancestor);
[[nodiscard]] bool contains_focus(QWidget& widget);
// inline bool contains_focus(Gtk::Widget* widget) { return false; }
...
QString get_synthetic_object_name(SPObject const* object);
...
// Qt image conversion function using Cairo surface image object.
// QImage will hold on to surface image sharing its data;
// the surface will be freed when the QImage is destroyed.
QImage cairo_surface_to_qimage(cairo_surface_t* surface);

// Qt widget utilities: drop shadow
void add_drop_shadow(QWidget* widget, int offset_x = 0, int offset_y = 1, int blur_radius = 16, int alpha = 90);

struct OverrideCursor {
    OverrideCursor(Qt::CursorShape shape) {
