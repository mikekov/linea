...
GTK-specific end */
/* QT-specific start */
#include "qt/qt-event-converter.h"
...
public:
    ...
    bool schedule_redraw_queued = false;
    ...
    // For tracking the old size during resize.
    ...
    /* QT-specific start */
    std::unique_ptr<QTimer> tick_timer;
    std::unique_ptr<QElapsedTimer> frame_timer;
    /* QT-specific end */
    Geom::Point strain;
    ...
    // QT specific:
    // Intermediate FBO that GLGraphics composites into (it requires COLOR_ATTACHMENT0)
    GLuint _widgetFbo       = 0;
    GLuint _widgetColorRb   = 0;  // renderbuffer — avoids Metal glTexImage2D restrictions
    GLuint _widgetStencil   = 0;
    int    _widgetFboW      = 0;
    int    _widgetFboH      = 0;
    uint32_t _pageColor     = 0xffffffff;
    uint32_t _deskColor     = 0xd4d4d4ff;
    uint32_t _borderColor   = 0x00000040;
};
    ...
    : d(std::make_unique<CanvasPrivate>(this))
{
    ...
    GTK-specific end */
    /* QT-specific start */
    // Qt: Events handled via virtual event() method override
    // Updater
    /* QT-specific end */
    ...
    /* QT-specific start */
    // Qt: get_realized() stub always returns true
    /* QT-specific end */
    ...
    GTK-specific end */
    /* QT-specific start */
    setMouseTracking(true); // make mouse move fire even without button pressed
    setTabletTracking(true);
    setFocusPolicy(Qt::StrongFocus);
    // disable IM, or else OS will keep on hiding mouse cursor when some keys are pressed
    setAttribute(Qt::WA_InputMethodEnabled, false); // disable input method (IM) events by default
    ...
    //QT specific
    d->sync.set_post_to_main([this](std::function<void()> f) {
        QMetaObject::invokeMethod(this, [this, f]() { f(); }, Qt::QueuedConnection);
    });
    setupCanvasItems();
}
...
/* QT-specific start */
// Qt: No explicit realize/unrealize needed - handled by QOpenGLWindow lifecycle
void Canvas::blink() {
    // Qt: TODO - implement blink effect
}
/* QT-specific end */
...
void CanvasPrivate::schedule_redraw(bool instant)
{
    ...
    if (redraw_active) {
        if ((schedule_redraw_conn || schedule_redraw_queued) && instant) {
    ...
    auto callback = [this] {
        ...
        if (!schedule_redraw_queued) return;
    ...
    if (instant) {
        ...
        GTK-specific end */
        /* QT-specific start */
        // Qt: Use QMetaObject::invokeMethod for idle-like behavior
        schedule_redraw_queued = QMetaObject::invokeMethod(q, callback, Qt::QueuedConnection);
        /* QT-specific end */
    }
}
...
void CanvasPrivate::launch_redraw()
{
    ...
    // Stores.
    auto world_area = q->get_area_world();
    handle_stores_action(stores.update(Fragment{ q->_affine, world_area }));
    // Geometry.
...
void CanvasPrivate::after_redraw() {
    ...
    // Process any geometry updates from deferred changes
...
void CanvasPrivate::autoscroll_begin(Geom::Point const &to)
{
    ...
    GTK-specific end */
    /* QT-specific start */
    // Qt: Use QTimer for tick callback and QElapsedTimer for frame timing
    if (!tick_timer) {
        tick_timer = std::make_unique<QTimer>(q);
        tick_timer->setTimerType(Qt::PreciseTimer);
        frame_timer = std::make_unique<QElapsedTimer>();
        QObject::connect(tick_timer.get(), &QTimer::timeout, q, [this]() {
        // Calculate dt using QElapsedTimer (similar to Gdk::FrameClock)
        qint64 t = frame_timer->elapsed();
        double dt;
        if (last_time) {
            dt = t - *last_time;
        } else {
            dt = 16.67; // Default 60fps ~16.67ms
        }
        last_time = t;
        dt *= 60.0 / 1e3 * prefs.autoscrollspeed; // Qt uses ms not us, so 1e3 not 1e6
        ...
        // Continue ticking (timer is already repeating)
    });
    }
                                                  ...
                                                  int n_press, double x, double y)
{
    ...
    if (controller.get_current_button() == 3) {
        _drawing->getCanvasItemDrawing()->set_sticky(_state & INK_SHIFT_MASK);
    }
...
/* QT-specific start */
// Qt: Event handlers called from Qt event() method
bool Canvas::on_scroll(double dx, double dy)
{
    _state = static_cast<int>(QGuiApplication::keyboardModifiers());

    auto event = ScrollEvent();
    event.modifiers = _state;
    event.device = nullptr;
    event.delta = { dx, dy };
    // Qt: unit has default value PIXEL in struct
    // Qt: extinput not available from QWheelEvent, use defaults
    event.extinput = {};
...
{
    // Convert QWheelEvent to scroll delta
    QPointF pixelDelta = event->pixelDelta();
    QPoint angleDelta = event->angleDelta();
    double dx = 0.0;
    double dy = 0.0;

    // Prefer pixel delta if available, otherwise convert angle delta
    if (!pixelDelta.isNull()) {
        dx = -pixelDelta.x() / 10.0;
        dy = -pixelDelta.y() / 10.0;
    } else {
        // Standard wheel: 15 degrees per step, 120 units per step
        dx = angleDelta.x() / 120.0 * 15.0;
        dy = angleDelta.y() / 120.0 * 15.0;
    }
...
void Canvas::on_button_pressed(ButtonPressEvent& event, int n_press)
{
    _state = event.modifiers;
    d->last_mouse = event.orig_pos;
    d->unreleased_presses |= 1 << event.button;

    if (event.button == 3) {
        // Qt: TODO - handle shift modifier for sticky
    }
    ...
    if (_split_mode == SplitMode::SPLIT && _hover_direction != SplitDirection::NONE) {
        if (n_press == 1) {
            _split_dragging = true;
            _split_drag_start = Geom::IntPoint(event.orig_pos.x(), event.orig_pos.y());
            return;
        } else if (n_press == 2) {
            _split_direction = _hover_direction;
            _split_dragging = false;
...
void Canvas::on_button_released(ButtonReleaseEvent& event)
{
    _state = event.modifiers;
    d->last_mouse = event.orig_pos;
    d->unreleased_presses &= ~(1 << event.button);
    ...
    if (_split_mode == SplitMode::SPLIT && _split_dragging) {
        ...
        // Check if we are near the edge. If so, revert to normal mode.
        // Qt: TODO - implement get_allocation equivalent or use canvas dimensions
        if (event.orig_pos.x() < 5 || event.orig_pos.y() < 5 ||
            event.orig_pos.x() > get_width() - 5 || event.orig_pos.y() > get_height() - 5)
        {
            // Reset everything.
            update_cursor();
            set_split_mode(SplitMode::NORMAL);
            // Qt: TODO - update action state
        }
    }
    ...
    if (d->unreleased_presses == 0 && d->delayed_leave_event) {
        ...
        auto leaveEvent = LeaveEvent();
        leaveEvent.modifiers = _state;

        d->process_event(leaveEvent);
    }
}

void Canvas::on_enter(double x, double y, int state)
{
    if (d->delayed_leave_event) {
        d->delayed_leave_event = false;
        return;
...
void Canvas::on_leave(int state)
{
    if (d->unreleased_presses != 0) {
        d->delayed_leave_event = true;
        return;
...
void Canvas::mousePressEvent(QMouseEvent* event) {
    auto canvasEvent = Linea::convertMousePress(event, this);

    // Qt delivers double-clicks to mousePressEvent as a separate event type
    // (when mouseDoubleClickEvent is not overridden). Mirror GTK's GestureClick
    // n_press counter so tools can detect multi-clicks.
    int n_press = (event->type() == QEvent::MouseButtonDblClick) ? 2 : 1;

    on_button_pressed(*canvasEvent, n_press);
    event->accept();
...
void Canvas::tabletEvent(QTabletEvent* event) {
    switch (event->type()) {
        case QEvent::TabletPress: {
            auto canvasEvent = Linea::convertTabletPress(event, this);
            on_button_pressed(*canvasEvent, 1);
            setFocus();
            break;
        }
        case QEvent::TabletMove: {
            auto canvasEvent = Linea::convertTabletMotion(event, this);
            on_motion(*canvasEvent);
            break;
        }
        case QEvent::TabletRelease: {
            auto canvasEvent = Linea::convertTabletRelease(event, this);
            on_button_released(*canvasEvent);
            break;
        }
        default:
            event->ignore();
...
void Canvas::mouseReleaseEvent(QMouseEvent* event) {
    auto canvasEvent = Linea::convertMouseRelease(event, this);
...
void Canvas::mouseMoveEvent(QMouseEvent* event) {
    auto canvasEvent = Linea::convertMotion(event, this);
    on_motion(*canvasEvent);
    event->accept();
}

void Canvas::setIMCursorRect(QRect rect) {
    _im_cursor_rect = rect;
...
void Canvas::inputMethodEvent(QInputMethodEvent* event) {
    if (!event->commitString().isEmpty()) {
        _signal_im_commit.emit(event->commitString());
    }
    event->accept();
}
...
bool Canvas::event(QEvent* event) {
    switch (event->type()) {
        case QEvent::ShortcutOverride: {
            // If the active tool wants all key input (e.g. text tool with cursor
            // placed), claim this key so Qt's shortcut map backs off and delivers
            // it as a normal keyPressEvent instead.
            if (_desktop) {
                if (auto tool = _desktop->getTool(); tool && tool->wantsAllKeys()) {
                    auto ke = dynamic_cast<QKeyEvent*>(event);
                    // Only claim keys that produce text or are navigation — not
                    // modifier-only presses which don't generate characters.
                    if (Linea::isTextInputKey(ke)) {
                        event->accept();
                        return true;
                    }
                }
        ...
        case QEvent::NativeGesture: {
            ...
            if (gestureEvent->gestureType() == Qt::ZoomNativeGesture) {
                double value = gestureEvent->value();
                // Formula: item.scale = item.scale * (1 + event.value)
                double scale = 1.0 + value;
                if (_desktop) {
                    _desktop->on_zoom_scale(scale);
                }
                event->accept();
                return true;
            }
...
bool Canvas::focusNextPrevChild(bool next) {
    // skip default widget handling to have 'tab' key delivered;
    // we cycle thru objects with 'tab' key instead
    return false;
}
                            ...
                            Gdk::ModifierType state)
{
    ...
    if (keyval == INK_KEY_VoidSymbol) return false;
...
void Canvas::on_key_released(int keyval, int keycode, int state)
{
    _state = static_cast<int>(state);

    auto event = KeyReleaseEvent();
    event.modifiers = _state;
    // Qt: device info not available from event controller, use defaults
    event.device = nullptr;
    event.keyval = keyval;
    event.keycode = keycode;
    event.group = 0;
...
void Canvas::keyReleaseEvent(QKeyEvent* event) {
    auto canvasEvent = Linea::convertKeyRelease(event);
...
void Canvas::on_motion(Gtk::EventControllerMotion const &controller, double x, double y)
{
    ...
    if (!(_state & (INK_BUTTON1_MASK | INK_BUTTON2_MASK | INK_BUTTON3_MASK))) {
        d->autoscroll_end();
    }
...
/* QT-specific start */
void Canvas::on_motion(MotionEvent& event) {
    _state = event.modifiers;

    auto const mouse = event.pos;
    if (mouse == d->last_mouse) {
        return; // Discard spurious motion events
    }
    d->last_mouse = mouse;

    if (on_mouse_moved) {
        on_mouse_moved(QPointF(mouse.x(), mouse.y()));
    }
    ...
    // Avoid embarrassing neverending autoscroll in case the button-released handler somehow doesn't fire
    if (!(_state & (0x1 | 0x2 | 0x4))) { // Qt::LeftButton | Qt::RightButton | Qt::MiddleButton masks
        d->autoscroll_end();
    }
...
bool CanvasPrivate::repick()
{
    ...
    if (!q->_all_enter_events) {
        ...
        /* GTK-specific start
        button_down = q->_state & (INK_BUTTON1_MASK |
                                   INK_BUTTON2_MASK |
                                   INK_BUTTON3_MASK |
                                   INK_BUTTON4_MASK |
                                   INK_BUTTON5_MASK);
        GTK-specific end */
        /* QT-specific start */
        // Qt: Check button state from the converted event state.
        button_down = q->_state & (INK_BUTTON1_MASK |
                                   INK_BUTTON2_MASK |
                                   INK_BUTTON3_MASK |
                                   INK_BUTTON4_MASK |
...
Geom::IntRect Canvas::get_area_world() const
{
    ...
    // return Geom::IntRect(_pos.x(), _pos.y(), _pos.x() + dim.x(), _pos.y() + dim.y());
}
...
GTK-specific end */
/* QT-specific start */
void Canvas::size_allocate(int const width, int const height)
{
    // Qt: No parent call needed
...
// QOpenGLContext* Canvas::create_context()
// {
//     // Qt: OpenGL context managed by QOpenGLWindow
//     return QOpenGLContext::currentContext();
// }
/* QT-specific end */

/* GTK-specific start */
bool Canvas::paint_widget(const QOpenGLContext& context, Cairo::RefPtr<Cairo::Context>& ctx) {
    ...
    GTK-specific end */
    /* QT-specific start */
    if (d->schedule_redraw_queued) {
        d->schedule_redraw_queued = false;
        d->launch_redraw();
    }
    /* QT-specific end */
...
// Cairo::RefPtr<Cairo::ImageSurface> surface = Cairo::ImageSurface::create(Cairo::Surface::Format::ARGB32, get_width(), get_height());
// auto ctx = Cairo::Context::create(surface);
    d->graphics->paint_widget(Fragment{ _affine, get_area_world() }, args, ctx);
...
// Qt-specific method stubs
void Canvas::setDesktop(SPDesktop* desktop) { _desktop = desktop; }

class CanvasItemContext* Canvas::canvasItemContext() const { return &d->canvasitem_ctx.value(); }

void Canvas::setupCanvasItems() {
    // Skip if already set up (idempotent)
    // if (_ready) {
        // return;
    // }

    // Create canvas item groups matching SPDesktop structure
    // These are the Inkscape CanvasItem scene graph nodes

    // Create context (nullptr canvas for Qt - no GTK widget available).
    // This auto-creates the root CanvasItemGroup internally.
    // _canvasItemContext = std::make_unique<CanvasItemContext>(this);
    CanvasItemGroup* root = canvasItemContext()->root();
    root->set_name("CanvasItemRoot");

    // Catchall must be the LOWEST item (first child) to catch all unhandled events
    // This matches GTK behavior where catchall is created before other groups
    // The catchall contains all points, so it must be at the bottom of the z-order
    // to allow other items to be picked first
    _canvas_catchall = new CanvasItemCatchall(root);
    // _canvas_catchall->set_name("CanvasItemCatchall");

    // Page background and foreground
    _canvas_group_pages_bg = new CanvasItemGroup(root);
    // Group for actual SVG drawing
    _canvas_group_drawing = new CanvasItemGroup(root);
    // page border
    _canvas_group_pages_fg = new CanvasItemGroup(root);
    // Group for grids
    _canvas_group_grids = new CanvasItemGroup(root);
    // Group for guides
    _canvas_group_guides = new CanvasItemGroup(root);
    // Group for sketch/temporary drawing
    _canvas_group_sketch = new CanvasItemGroup(root);
    // Group for temporary items
    _canvas_group_temp = new CanvasItemGroup(root);
    // Root group for controls (handles, knots, nodes) - MUST BE LAST to be on top in z-order
    _canvas_group_controls = new CanvasItemGroup(root);
    ...
    // The CanvasItemDrawing will be created by QtDesktopAdapter when document is set
}

CanvasItemGroup* Canvas::canvasGroupControls() const { return _canvas_group_controls; }
CanvasItemGroup* Canvas::canvasGroupDrawing() const { return _canvas_group_drawing; }
CanvasItemGroup* Canvas::canvasGroupGrids() const { return _canvas_group_grids; }
CanvasItemGroup* Canvas::canvasGroupGuides() const { return _canvas_group_guides; }
CanvasItemGroup* Canvas::canvasGroupTemp() const { return _canvas_group_temp; }
CanvasItemGroup* Canvas::canvasGroupSketch() const { return _canvas_group_sketch; }

void Canvas::setColours(uint32_t page, uint32_t desk, uint32_t border) {
    d->page = page;
    d->desk = desk;
    // d->border = border;
    queue_draw();
...
Geom::Point Canvas::windowToCanvas(const QPointF& windowPos) const {
    return Geom::Point(windowPos.x(), windowPos.y());
}

QPoint Canvas::canvasToWindow(const Geom::Point& canvasPos) const {
    return QPoint(static_cast<int>(canvasPos.x()), static_cast<int>(canvasPos.y()));
}

Geom::Affine Canvas::windowToCanvasTransform() const {
    return canvasToWindowTransform().inverse();
}

Geom::Affine Canvas::canvasToWindowTransform() const {
    return Geom::Affine(_affine);
}

void Canvas::set_cursor(const std::string& cursor_name) {
    // SVG cursor files (e.g. "select.svg", "rotate.svg") — load and render.
    if (cursor_name.size() >= 4 &&
        cursor_name.compare(cursor_name.size() - 4, 4, ".svg") == 0) {
        setCursor(Inkscape::load_svg_cursor_qt(cursor_name, devicePixelRatioF()));
    }
    else {
        g_warning("Canvas: Unknown cursor type requested: '%s'", cursor_name.c_str());
    }
}

void Canvas::set_cursor(Qt::CursorShape shape) {
    setCursor(shape);
}

void Canvas::grab_focus() {
    setFocus();
}

bool Canvas::has_focus() const {
    return hasFocus();
}

void Canvas::handleKeyPress(QKeyEvent* event) {
    keyPressEvent(event);
    // on_key_pressed(event->key(), 0, static_cast<int>(event->modifiers()));
}

// Qt-specific OpenGL helpers
void Canvas::ensureWidgetFbo(int w, int h) {
    if (d->_widgetFbo && d->_widgetFboW == w && d->_widgetFboH == h) return;

    // Use renderbuffers for both color and stencil — avoids macOS Metal's
    // glTexImage2D restriction on textures with FBO attachment history.
    if (d->_widgetFbo)     { glDeleteFramebuffers(1,  &d->_widgetFbo);     d->_widgetFbo = 0; }
    if (d->_widgetColorRb) { glDeleteRenderbuffers(1, &d->_widgetColorRb); d->_widgetColorRb = 0; }
    if (d->_widgetStencil) { glDeleteRenderbuffers(1, &d->_widgetStencil); d->_widgetStencil = 0; }
    while (glGetError() != GL_NO_ERROR) {}

    glGenRenderbuffers(1, &d->_widgetColorRb);
    glBindRenderbuffer(GL_RENDERBUFFER, d->_widgetColorRb);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_RGBA8, w, h);

    glGenRenderbuffers(1, &d->_widgetStencil);
    glBindRenderbuffer(GL_RENDERBUFFER, d->_widgetStencil);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, w, h);
    while (glGetError() != GL_NO_ERROR) {}

    glGenFramebuffers(1, &d->_widgetFbo);
    glBindFramebuffer(GL_FRAMEBUFFER, d->_widgetFbo);
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,        GL_RENDERBUFFER, d->_widgetColorRb);
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, d->_widgetStencil);

    GLenum status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
    if (status != GL_FRAMEBUFFER_COMPLETE) {
        glDeleteFramebuffers(1,  &d->_widgetFbo);     d->_widgetFbo = 0;
        glDeleteRenderbuffers(1, &d->_widgetColorRb); d->_widgetColorRb = 0;
        glDeleteRenderbuffers(1, &d->_widgetStencil); d->_widgetStencil = 0;
        return;
    }
