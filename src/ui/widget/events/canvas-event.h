...
struct ButtonEvent : CanvasEvent
{
    ...
    {
        switch (button) {
            case 1: return INK_BUTTON1_MASK;
            case 2: return INK_BUTTON2_MASK;
            case 3: return INK_BUTTON3_MASK;
            case 4: return INK_BUTTON4_MASK;
            case 5: return INK_BUTTON5_MASK;
...
struct KeyEvent : CanvasEvent
{
    unsigned modifiersChange() const override
    {
        switch (keyval) {
            ...
            case INK_KEY_Shift_R:
                return INK_SHIFT_MASK;
            case INK_KEY_Control_L:
            case INK_KEY_Control_R:
                return INK_CONTROL_MASK;
            case INK_KEY_Alt_L:
            case INK_KEY_Alt_R:
                return INK_ALT_MASK;
            case INK_KEY_Meta_L:
            case INK_KEY_Meta_R:
                return INK_META_MASK;
            default:
                return 0;
        }
    }
    ...
    /* QT-specific start */
    /// The printable text generated by this key press, if any (Qt: QKeyEvent::text()).
    std::string text;

    /// The Qt key code (Qt::Key) from the original QKeyEvent.
    int qtKey = 0;

    /// The Qt keyboard modifiers (Qt::KeyboardModifiers) from the original QKeyEvent.
    unsigned qtModifiers = 0;
    /* QT-specific end */
};
...
struct ScrollEvent final : CanvasEvent
{
    ...
    ScrollUnit unit = ScrollUnit::SURFACE;
...
inline bool mod_shift(unsigned modifiers) { return modifiers & INK_SHIFT_MASK; }
inline bool mod_ctrl(unsigned modifiers) { return modifiers & INK_CONTROL_MASK; }
inline bool mod_alt(unsigned modifiers) { return modifiers & INK_ALT_MASK; }
inline bool mod_shift_only(unsigned modifiers) { return (modifiers & INK_MODIFIER_MASK) == INK_SHIFT_MASK; }
inline bool mod_ctrl_only(unsigned modifiers) { return (modifiers & INK_MODIFIER_MASK) == INK_CONTROL_MASK; }
inline bool mod_alt_only(unsigned modifiers) { return (modifiers & INK_MODIFIER_MASK) == INK_ALT_MASK; }
inline bool mod_any(unsigned modifiers) { return modifiers & INK_MODIFIER_MASK; }
inline bool mod_none(unsigned modifiers) { return !mod_any(modifiers); }

template <unsigned button>
inline bool mod_button(unsigned modifiers) { return (1 <= button || button <= 5) && (modifiers & (INK_BUTTON1_MASK << (button - 1))); }
