...
public:
    ...
    std::optional<Glib::ustring> cssVarValue() const {
        return css_var ? std::make_optional(*css_var) : std::nullopt;
    }
  // To do: make private
public:
    ...
    std::shared_ptr<Glib::ustring> css_var; // Unresolved var(--name) token; null for most properties.
