...
/**
 * Non-owning view into ARGB32 premultiplied pixel data (Cairo's native format).
 * This is the format Inkscape::Pixbuf stores images in for rendering.
 * On little-endian, bytes in memory are: B, G, R, A per pixel.
 */
struct BitmapView {
    int width;
    int height;
    int rowstride;
    constexpr static int channels = 4; // always 4 for ARGB32
...
public:
    ...
    /**
     * Take an ARGB32 premultiplied bitmap, trace it, and return a style attribute and the
     * path data that is compatible with the d="" attribute of an SVG <path> element.
     *
     ...
     */
    virtual TraceResult trace(const BitmapView& bmp, Async::Progress<double>& progress) = 0;

    /**
     * Generate a quick preview without any actual tracing. Like trace(), this must be thread-safe.
     */
    virtual QImage preview(const BitmapView& bmp) = 0;
...
TraceFuture preview(std::unique_ptr<TracingEngine> engine,
                    bool sioxEnabled,
                    std::function<void(QImage)> onfinished);
