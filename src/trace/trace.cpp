...
bool confirm_image_size(TracingEngine const *engine, Geom::IntPoint const &dimensions)
{
    if (engine->check_image_size(dimensions)) {
        QString message = QString::fromUtf8(
            _("Image looks too big. Process may take a while and it is"
        ...
        msgBox.setWindowTitle(QString::fromUtf8(_("Warning")));
...
public:
    ...
    QImage process(const SioxImage& sioximage, Async::Progress<double>& progress) const;

private:
    mutable std::mutex mutables;
    mutable unsigned last_hash = 0;
    mutable QImage last_result;

    SioxImageCache() = default;
};

QImage SioxImageCache::process(const SioxImage& sioximage, Async::Progress<double>& progress) const
{
    ...
    last_hash = hash;
    last_result = result.toQImage();

    return last_result;
}

QImage sioxProcessImage(const BitmapView& bmp, Cairo::RefPtr<Cairo::ImageSurface> siox_mask, Async::Progress<double>& progress)
{
    // Copy the bitmap into the siox image.
    auto sioximage = SioxImage(bmp);
...
public:
    ...
    TraceTask(std::unique_ptr<TracingEngine> engine, bool sioxEnabled, std::function<void(QImage)> onfinished)
        : engine(std::move(engine))
        , sioxEnabled(sioxEnabled)
        , type(Type::Preview)
        , onprogress([] (auto&&) {})
...
private:
    ...
    std::function<void(QImage)> onfinished_preview; // For preview task.
...
TraceFuture preview(std::unique_ptr<TracingEngine> engine, bool sioxEnabled, std::function<void(QImage)> onfinished)
{
...
void TraceTask::do_async_work(std::unique_ptr<TraceTask> self)
{
    ...
    try {
        ...
        // Create a BitmapView directly from the immutable Inkscape::Pixbuf data.
        // SPImage always stores images in PF_CAIRO (ARGB32 premultiplied), so no
        // copy or format conversion is needed.
        BitmapView bmp{
            image_pixbuf->width(),
            image_pixbuf->height(),
            image_pixbuf->rowstride(),
            image_pixbuf->pixels()
        };

        // If SIOX has been enabled, run SIOX processing.
        // The SIOX result is a QImage (ARGB32 premultiplied); create a BitmapView
        // from it to feed into the engine.
        QImage siox_result;
        if (sioxEnabled) {
            siox_result = sioxProcessImage(bmp, siox_mask, *sub_siox);
            siox_mask.reset();
            sub_siox->report_or_throw(1.0);
            bmp = BitmapView{
                siox_result.width(),
                siox_result.height(),
                static_cast<int>(siox_result.bytesPerLine()),
                siox_result.constBits()
            };
        }

        // If in preview mode, compute and return the preview and exit now.
        if (type == Type::Preview) {
            QImage result = engine->preview(bmp);
            channel.run(std::bind(onfinished_preview, std::move(result)));
            return;
        }

        // Actually perform the tracing.
        traceresult = engine->trace(bmp, *sub_trace);
        ...
        if (type == Type::Trace) {
            ...
            channel.run(std::bind(onfinished_preview, QImage()));
        }
