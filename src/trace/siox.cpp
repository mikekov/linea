...
SioxImage::SioxImage(const BitmapView& bmp) {
    width = bmp.width;
    height = bmp.height;

    // Copy pixel data, un-premultiplied from ARGB32 premultiplied to straight ARGB.
    // SIOX performs color clustering in CIELAB space, so it needs non-premultiplied values.
    pixdata = bitmapToArgb(bmp);

    // Zero confidence matrix.
    cmdata.resize(pixdata.size());
    std::fill(cmdata.begin(), cmdata.end(), 0.0f);
}

QImage SioxImage::toQImage() const {
    return argbToQImage(pixdata.data(), width, height);
}
