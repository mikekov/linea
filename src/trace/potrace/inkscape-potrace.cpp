...
std::optional<GrayMap> PotraceTracingEngine::filter(const BitmapView& bmp) const
{
    ...
    if (traceType == TraceType::QUANT) {
        ...
        auto rgbmap = bitmapToRgbMap(bmp);
...
IndexedMap PotraceTracingEngine::filterIndexed(const BitmapView& bmp) const
{
    auto map = bitmapToRgbMap(bmp);
...
QImage PotraceTracingEngine::preview(const BitmapView& bmp)
{
    ...
    {
        auto gm = filterIndexed(bmp);

        return indexedMapToQImage(gm);
 ...
 */
TraceResult PotraceTracingEngine::traceSingle(const BitmapView& bmp, Async::Progress<double>& progress)
{
    brightnessFloor = 0.0; // important to set this, since used by filter()

    auto grayMap = filter(bmp);
    if (!grayMap) {
        return {};
    }
 ...
 */
TraceResult PotraceTracingEngine::traceBrightnessMulti(const BitmapView& bmp, Async::Progress<double>& progress)
{
 ...
 * Quantization
 */
TraceResult PotraceTracingEngine::traceQuant(const BitmapView& bmp, Async::Progress<double>& progress)
{
    auto imap = filterIndexed(bmp);
...
TraceResult PotraceTracingEngine::trace(const BitmapView& bmp, Async::Progress<double>& progress)
{
    if (traceType == TraceType::QUANT_COLOR || traceType == TraceType::QUANT_MONO) {
        return traceQuant(bmp, progress);
    } else if (traceType == TraceType::BRIGHTNESS_MULTI) {
        return traceBrightnessMulti(bmp, progress);
    } else {
        return traceSingle(bmp, progress);
    }
}
