 ...
 */
class Selection : public QObject, public ObjectSet
{
    Q_OBJECT
public:
    /**
    ...
    /**
     * Block/unblock modified signal emission.
     * Used during drag operations to prevent feedback loops.
     */
    void blockModified() { _modified_block_count++; }
    void unblockModified() { if (_modified_block_count > 0) _modified_block_count--; }
...
private:
    ...
    unsigned _flags = 0;
    QTimer *_modified_timer = nullptr;
    bool _change_layer = true;
    bool _change_page = true;
    int _modified_block_count = 0;  // Block counter for modified signal emission during drag
